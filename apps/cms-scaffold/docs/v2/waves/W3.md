...
- **公開導覽 API**（`GET /api/v1/public/navigation/{menuKey}`）：種子只有 `front.primary` 一份選單，狀態是草稿、從未發布、英文標籤、跨站連結（`services/cms-api/src/main/java/com/fallrising/cms/content/service/ContentTypeSeed.java` 第 115～129 行），公開 API 永遠回 404 `NAVIGATION_NOT_FOUND`。本波用 `sites.ts` 寫死的各站導覽（surface-front §6.3 允許的 fallback），不呼叫這個 API → [01 Q-18](../01-frontend-sdd.md#136-w3-細化時新增待確認)。
- **enum 顯示名稱從 API 來**：公開契約的 `PublicField` 沒有 `enumLabels`；本波把三個欄位的標籤放在 `copy.ts`（`enum.<type>.<field>.<value>`）→ [01 Q-17](../01-frontend-sdd.md#136-w3-細化時新增待確認)。
