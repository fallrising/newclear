...
>
> 本波的起點是 **W2 已實作完成、W3 尚未實作**的程式（[W2.md](W2.md) §5 的結果；W2 以 W1、W1 以 W0 為起點）。§5 列為「新增」「取代」的檔案，以本檔全文建立或整個覆蓋；列為「修改」的檔案，只套用 §5 寫出的 diff（`patch -p1` 可直接套用，行號以 W2 的結果為準）。
>
> **預演紀錄。** 預演的做法：從 `main` 取 W0.md、W1.md、W2.md §5 的全文與 diff，依三份施工圖的任務卡重建出「W2 完成後」的程式（W0 的 fixture 取自提交 `036aedf`、W1 取自 `884cfb3`，模擬當時的 `contracts/fixtures/`），逐波確認完整閘門全綠：W0 Vitest 118／`e2e:mock` 17，W1 197／27，W2 256／39，與各施工圖記載的數字相同。再在 W2 的結果上實作本波，跑完 §9 的完整閘門（Vitest 324、`e2e:mock` 64）。本檔 §5 的區塊由預演結果直接產生；另以「從 W2 結果逐張套用本檔任務卡」的方式重放一次，每張「測試先行」卡的紅燈是實際跑出來的（§6）。
...
| V2-AC-14 | 01 §11.2 | Admin 17 個畫面與 4 種對話框 axe critical／serious 為 0 | T22 | `admin-w4.spec.ts` ×17＋對話框 ×3 |
| V2-AC-15 | 01 §11.2 | 新文案不含工程詞；17 個畫面掃描 | T22 | `admin-w4.spec.ts` ×17；`shell.test.tsx`、`types.test.tsx` |
...
| surface-admin AC-B | surface-admin §12 | 啟用、停用類型（確認對話框）；停用確認文案不含已發布數（API 沒有，[01 Q-17](../01-frontend-sdd.md#136-w4-細化時新增待確認)） | T13 | `types.test.tsx` ×3 |
...
| surface-admin AC-D（部分） | surface-admin §12、§6.3 | 建立帳號並指派 `operator`＋診所四個類型（流程 B 以系統角色＋帳號類型範圍完成；建立自訂角色 `clinic_operator` 沒有 API，[01 Q-18](../01-frontend-sdd.md#136-w4-細化時新增待確認)） | T16 | `principals.test.tsx` flow B；`admin-w4.spec.ts` flow B |
...
| surface-admin AC-G（條目） | surface-admin §12、§8 | 永久刪除條目要輸入網址代稱；被引用時 409 → toast，資料不動。媒體硬刪沒有 API（[01 Q-19](../01-frontend-sdd.md#136-w4-細化時新增待確認)） | T20 | `entries.test.tsx` ×2 |
...
- **自訂角色的建立、複製、改名、刪除**：BW5 只有 `GET /roles`、`GET`／`PUT /roles/{code}/permissions`，沒有 `POST`／`PATCH`／`DELETE /roles` → [01 Q-18](../01-frontend-sdd.md#136-w4-細化時新增待確認)。`/roles/new` 不做；角色詳情沒有「擁有這個角色的帳號」（沒有 API 可以查）。
- **類型的已發布數、條目數、pack 分組與批次啟停、`defaultVisibility`、`surfaces`**：`AdminContentType` 沒有這些欄位、也沒有對應的寫入端點 → [01 Q-17](../01-frontend-sdd.md#136-w4-細化時新增待確認)。停用確認文案不含「目前 published = {n}」。
- **依帳號的媒體用量、調整配額、媒體硬刪、`/settings/storage`、`/settings/security`、`/media/:id`**：契約只有 `GET /media/quota`（呼叫者所見的整個媒體庫）→ [01 Q-19](../01-frontend-sdd.md#136-w4-細化時新增待確認)。`/media` 只顯示整體用量。
- **帳號列表的角色欄、最後登入時間、總覽的「只剩 1 位管理員」警示、會員連結對話框只列會員**：`Principal` 沒有角色與 `lastLoginAt`，`GET /principals` 沒有角色篩選 → [01 Q-20](../01-frontend-sdd.md#136-w4-細化時新增待確認)。
- **清除帳號的電子郵件**：`PatchPrincipalRequest` 的 `null` 代表「不變」，沒有清除的寫法 → [01 Q-21](../01-frontend-sdd.md#136-w4-細化時新增待確認)。畫面提示「目前無法清除電子郵件」。
- **後端的危險操作閘門**（surface-admin §8 的 `confirmPhrase`／`confirmId`、`SELF_DEMOTION_FORBIDDEN`、`SELF_DISABLE_FORBIDDEN`）：BW5 沒有；本波只在前端擋 → [01 Q-22](../01-frontend-sdd.md#136-w4-細化時新增待確認)。
...
| W3 | **尚未**實作 | W3 也會改 `packages/api`、`packages/mocks`（codegen 同樣要換到 BW5）。W3 先合併時本檔的 diff 會衝突 → 停下來回報，交給下一次細化以 W3 的結果重新產生 §5 的 diff |
...
三行輸出必須依序是 `0.11.0`、`1`、`apps/web-back/.env`。第一行不同 → 契約已改版；第二行不是 `1` → 起點不是 W2 的結果（例如 W3 已實作）；第三行空白 → W1 的 `.env` 沒有提交（[W1.md](W1.md) 已補註：`.gitignore` 的 `.env` 規則會忽略它，要 `git add -f`）。任何一項不符就停下來回報。
...
| `e2e-mock/shell.spec.ts` | 修改 | Admin 卡片樣式改在總覽量測 | T22 |
| `e2e-mock/admin-w4.spec.ts` | 新增 | 25 個 Playwright 測試 | T22 |
| `docs/v2/README.md` | 修改 | 路線圖 W4 列狀態改 `VERIFIED` | T23 |

不會碰：`apps/web-front/**`、`apps/web-back/**`、`packages/auth/**`、`packages/ui/src/patterns/*.tsx`（新增測試檔除外）、`package.json`、`package-lock.json`、`playwright.mock.config.ts`、`eslint.config.mjs`、`scripts/**`、`services/**`、`.github/**`、`docs/**`（`docs/v2/README.md` 的 W4 狀態除外）。
...
本波**不新增** API；用到的 operation 全部在 [`contracts/BW5.openapi.yaml`](../contracts/BW5.openapi.yaml)。沒有 `contracts/W4.openapi.yaml`。surface-admin §9.2 的草案路徑（`/audit-events`、`/settings/media`、`PATCH /content-types/{slug}`…）以 BW5 契約的實際路徑取代；缺的端點列在 §1.2 與 01 §13.6。下表是本波呼叫的全部 operation、呼叫點與錯誤處理。實作者照做，不要另加處理。
...
`shell.spec.ts` 的 Admin 樣式測試改在總覽量測（`/types` 變成表格，沒有卡片）；其他不變。`admin-w4.spec.ts` 的 25 個測試見 §7.9。
...
- **目標**：瀏覽器中的 axe、危險操作、矩陣與流程。
- **輸入**：T21。
- **步驟**：套用 §5.6 的 `shell.spec.ts` diff；以全文建立 `e2e-mock/admin-w4.spec.ts`；確認 5173～5175 沒有其他程式占用，執行 `npm run e2e:mock`。
- **完成條件**：64 個測試全綠（W2 的 39 個＋本波 25 個）。
- **驗證**：`npm run e2e:mock`
- **對應 ID**：V2-AC-14、V2-AC-15、V2-AC-16、AC-D、AC-E、AC-K、01 Q-13
- **預估大小**：M
...
- **步驟**：
  ...
  3. 開 PR，標題 `feat(cms-scaffold): v2 W4 Admin 治理面`；說明列出：解決的 ID（§1.1）、實際跑過的指令與結果、只在 CI 執行的檢查（CI 的 `e2e:mock` 瀏覽器安裝）、§1.2 的不做與對應的 01 Q-17～Q-22。
...
### 7.9 `e2e-mock/admin-w4.spec.ts`（P，25 個）與 `shell.spec.ts`（P，修改 1 個）
...
| W4-FM22 | 自己的帳號：停用、移除管理員角色 | 前端不提供控件；後端只有 `LAST_ADMIN`（[01 Q-22](../01-frontend-sdd.md#136-w4-細化時新增待確認)） | `principals.test.tsx` #5 | T16 |
...
```
  ...
  預期：Vitest 324（api 19、auth 38、fields 28、mocks 47、ui 18、web-admin 62、web-back 103、web-front 9）；`test:bundle passed: web-front, web-back, web-admin`；`e2e:mock` 64 passed。
...
- [ ] PR 說明列出實際跑過的指令與結果，以及只在 CI 執行的檢查。
