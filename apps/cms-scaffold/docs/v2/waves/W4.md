...
>
> **e2e 不在本波。** 依 owner 指示（2026-09-26，[01 Q-19](../01-frontend-sdd.md#136-w3-細化時新增已決定owner2026-09-26)），`e2e:mock`（Playwright）只在整個專案完成後執行：本波的任務卡與閘門都**不含** e2e、不新增 e2e 測試。唯一的例外是 `e2e-mock/shell.spec.ts` 的 Admin 樣式測試（`/types` 改成表格後，那個測試找不到卡片；不改的話專案完成時 CI 的 `web` job 會紅），T22 只改檔、不執行。預演時寫好、跑過的 W4 e2e 規格放在[附錄 A](#附錄-a專案完成後才執行的-e2e)，供專案完成後執行 e2e 的 agent 使用。
>
