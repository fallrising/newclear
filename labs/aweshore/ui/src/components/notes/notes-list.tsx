...
export const NotesList = component$(() => {
  ...
  const fetchNotes = $(async () => {
    ...
    try {
      const { notes, totalPages, totalCount, lastId } = await api.getNotes(
        store.currentPage,
        store.pageSize,
      );
      store.notes = notes;
  ...
  const notesResource = useResource$<Note[]>(async ({ track }) => {
    ...
    const { notes, totalCount, totalPages } = await api.getNotes(
      store.currentPage,
      store.pageSize,
    );
    store.notes = notes;
              ...
              <div class="level-item">
                <div class="tags has-addons">
                  ...
                  <span class="tag is-primary">
                    {store.totalPages.toLocaleString()}
                  </span>
                </div>
              </div>
            )}
            ...
            <tr>
              ...
              <td>
                <input
                  ...
                  onInput$={(e) =>
                    (store.newNote.title = (e.target as HTMLInputElement).value)
                  }
                />
              </td>
              <td>
                {" "}
                {/* Merge cells for content textarea */}
                <textarea
                  class="textarea"
                  placeholder="Content"
