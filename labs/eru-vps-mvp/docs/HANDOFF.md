...
更新：2026-09-26。固定剩餘任務編號以 TASKS.md 為準：仍為 12 項（近期 1、後續 11）；ERU-001～006 已完成，ERU-007 待正式 V11 小流量驗收。依 owner 指示先完成本機開發，再統一進行 VPS E2E。ERU-012 本機 desired-state 功能已齊待 E2E；ERU-013 provenance／版本 guard 已合併，既有 v0.1.5 patch 已以 Go 1.27.1 隔離重驗；ERU-014 已有 worker-only install executor／唯讀 reconcile，並完成安全 AddNode patch 的 Go 1.27.1 雙環境建置與本機 resume helper 測試。新 patch 尚未部署，也未接入 ERU-014 registration executor；registration、smoke、recovery 與 E2E 仍待開發，任務數未減。每完成正式任務須更新清單並回報編號與剩餘數。24h 低頻觀測已於 2026-09-24 11:25:06 UTC 結束，摘要見 TODO-SOAK-2026-09-23.md。
...
- ERU-014 本機前置進行中：人工控制台 intent／receipt、replacement identity gate、hash-bound preparation／planner、worker-only install executor 與唯讀 reconcile 均完成本機 fake 驗證；agent 停止、node 未註冊。追加 safe AddNode patch：pinned v0.1.5 新 node 現在預設 Bypass=true，並有 core/store 測試；Go 1.27.1 兩個隔離 build 的 regression／calcium+store／lock tests／build 均通過。新增 resume helper 僅在身份／endpoint 相符、仍 fenced 且 agent 可用時發出一次 node up，並檢查結果；6 項離線測試通過。patch 為 verified-not-deployed，helper 尚未接上 registration executor，沒有連 VPS 或修改 private inventory；仍缺受控 core 部署、重新納管流程、smoke、故障恢復與 E2E，總剩餘數仍為 12。詳見 M3-CORE-SAFE-NODE-ADD-2026-09-26.md。
