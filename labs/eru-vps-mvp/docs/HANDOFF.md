...
- ERU-014 本機前置進行中：[人工控制台重灌 intent](M3-REIMAGE-INTENT-2026-09-25.md) 將私有 owner review、worker machine ID、provider resource／OS image、精確 boot／attached volume 範圍及 intent hash 綁入唯讀計畫；有效計畫仍不可執行；新增的 owner receipt 只做 hash／schema／OOB fingerprint 聲明核對並本機 immutable 記錄，不驗證實機，也不更新 trust store 或重新納管；39 個定向測試及完整 288 tests 通過。Drain／重新納管／resume adapter 與新 host key／SSH／Tailscale／runtime bootstrap 尚未完成，沒有建立實機 intent／receipt，也沒有做 OS 重灌或 VPS mutation。
