...
更新：2026-09-26。固定剩餘任務編號以 TASKS.md 為準：仍為 12 項（近期 1、後續 11）；ERU-001～006 已完成，ERU-007 待正式 V11 小流量驗收。依 owner 指示先完成本機開發，再統一進行 VPS E2E。ERU-012 本機 desired-state 功能已齊待 E2E；ERU-013 provenance／版本 guard 已合併，既有 v0.1.5 patch 已以 Go 1.27.1 隔離重驗；ERU-014 已有 worker-only install、core access preparation、fenced registration／smoke／safe-resume executors 與唯讀 reconcile，safe AddNode patch 已完成 Go 1.27.1 雙環境建置；resume 成功停在 generation commit 前。patch 尚未部署；新 access stage 僅完成 fake 驗證、尚未 VPS 驗收，仍缺 resume 後 private inventory／worker IP／generation commit、跨階段 recovery 與 E2E，任務數未減。每完成正式任務須更新清單並回報編號與剩餘數。24h 低頻觀測已於 2026-09-24 11:25:06 UTC 結束，摘要見 TODO-SOAK-2026-09-23.md。
...
工作樹 `/home/ckc/test/codex/newclear-eru-delivery`，分支 `work/eru-014-worker-access`，專案 `labs/eru-vps-mvp`。其他工作樹的 Kith 未提交修改必須保留。Repo 本輪整併見根目錄 `docs/repo-consolidation-2026-09-22.md`；GitHub PR #29–#31 已合併。
...
- ERU-014 本機前置進行中：人工控制台 intent／receipt、replacement identity gate、hash-bound preparation／planner、worker-only install、registration 前 core known_hosts／nft access preparation、fenced registration、fenced smoke 與 safe-resume executors／唯讀 reconcile 均以 fake operator 驗證。access stage 只更新 target 的 SSH trust 與 source allowlist，不重啟 core；registration／smoke 要求相同 access proof。registration 核對 core running binary SHA，只 AddNode 為 bypass=true、啟動 agent 並等待 available=true；smoke 在 peer canaries 的連續 HTTP guards 下完成 target nginx lifecycle checks 並清理；resume 透過 ckc-disposable-01 SSH alias 單次執行 node up、核對 available=true／bypass=false，成功停在 `resumed-awaiting-generation-commit` 且不改 generation。各階段不確定時先唯讀 reconcile，不重播。safe AddNode patch：pinned v0.1.5 新 node 預設 Bypass=true，Go 1.27.1 兩個隔離 build 的 regression／calcium+store／lock tests／build 均通過。patch 仍 verified-not-deployed，未連 VPS 或修改 private inventory。仍缺受控 core 部署、resume 後 inventory／generation commit、跨階段 recovery 與 E2E；總剩餘數仍為 12。詳見 [safe AddNode](M3-CORE-SAFE-NODE-ADD-2026-09-26.md)、[core access](M3-REIMAGE-WORKER-ACCESS-2026-09-26.md)、[registration](M3-REIMAGE-WORKER-REGISTER-2026-09-26.md)、[smoke](M3-REIMAGE-WORKER-SMOKE-2026-09-26.md) 與 [resume](M3-REIMAGE-WORKER-RESUME-2026-09-26.md)。
