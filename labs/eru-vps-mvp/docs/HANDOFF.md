...
更新：2026-09-26。固定剩餘任務編號以 TASKS.md 為準：仍為 12 項（近期 1、後續 11）；ERU-001～006 已完成，ERU-007 待正式 V11 小流量驗收。依 owner 指示先完成本機開發，再統一進行 VPS E2E。ERU-009 已新增 ERU-012 managed stateless app 的離線 hash-bound drain planner（10 個 fake tests）、多 app staged executor／唯讀 recovery、partial cleanup fresh exact-ID plan／child journal reconciliation 與 `labctl` 操作入口；executor 先 fence、全部 replacement readiness 完成後才 exact-ID cleanup，只有完整 empty-worker audit 才開一般重裝 gate。ERU-009 有 26 個 drain fake tests；共享 `AppRevisionCleanup` 會在每次 exact-ID remove 後及最後 readiness probe 後比對全群 workload identity，reconcile 入口也會共用 `ClusterLock`，避免與 execute 並行覆寫 journal。identity drift 會停在 `needs_review`。目前完整本機 suite 383 tests 通過（PR #137 合併時為 381）。尚未連 VPS 或跑 E2E；cleanup 執行命令僅以 fake adapter 驗證。ERU-012 本機 desired-state 功能已齊待 E2E；planner／executor／adapter／cleanup 有 54 個離線測試。ERU-013 provenance／版本 guard 已合併，v0.1.5 patch 已以 Go 1.27.1 重驗；另對 unreleased upstream master `e9b48c1` 完成 forward-compatibility tests／兩次一致 build，尚無新 stable tag、未建立 manifest；ERU-014 已有 worker-only install、core access preparation、fenced registration／smoke／safe-resume、hash-bound inventory／generation commit 與跨階段 snapshot/recovery coordinator；safe AddNode patch 已完成 Go 1.27.1 雙環境建置，仍 verified-not-deployed。跨階段鏈結核對 predecessor journal hashes／target identity，恢復器只呼叫第一個不確定 stage 的既有唯讀 reconciler；generation commit 還會重核本機 hashes。上述 ERU-014 執行器仍為 fake fixture 驗證，未連 VPS 或讀寫真實 private data；ERU-009、ERU-013、ERU-014 仍進行中；整體 E2E 依 owner 指示排在本機開發之後，任務數未減。每完成正式任務須更新清單並回報編號與剩餘數。24h 低頻觀測已於 2026-09-24 11:25:06 UTC 結束，摘要見 TODO-SOAK-2026-09-23.md。
...
- ERU-009 本機 slice 仍進行中：`scripts/worker_drain.py` 以 exact app owner／digest／workload IDs 和明確 destination 建 review-only plan；`scripts/worker_drain_executor.py` 使用 ERU-012 `AppExecutor`／`AppRevisionCleanup`、one-shot target fence、跨 app readiness barrier、exact-ID cleanup 與 read-only recovery。partial cleanup 後會比對 parent snapshot 與 child cleanup reconciliation，fresh plan 只含仍存在的 exact source IDs；每次 cleanup 前需新對帳，不重播失敗 plan。共用 cleanup executor 每次 remove 後及最後 readiness probe 後核對全群 workload identity，異常時停止且不繼續移除來源；AppExecutor／AppRevisionCleanup reconcile 入口也使用 `ClusterLock`，drain child reconciliation 重用已持鎖路徑。planner 10 個、executor／recovery／adapter／`labctl` 16 個 fake tests，以及 ERU-012 cleanup/reconcile regressions 通過。完整 suite 現為 383 tests（PR #137 合併時 381）。沒有執行 VPS 命令或 E2E，ERU-009 仍進行中，剩餘數維持 12。詳見 [ERU-009 前置](M3-WORKER-NONEMPTY-DRAIN-PREP-2026-09-26.md)。
- ERU-012 本機功能已齊：digest-pinned stateless HTTP spec／review planner、hash-bound executor、EruCLIAdapter 與 exact-ID cleanup；沿用 labctl.Operator、固定 SSH aliases、雙階段 etcd/core/consistency preflight、digest image cache、bodyless worker probe；cleanup 每次 remove 前驗新版 readiness，並核對全群 workload identity；public reconcile 也在共用鎖內序列化 journal 更新。容量 admission 交由 Eru resource plugin，v1 不含外部 traffic routing。planner／executor／adapter／cleanup 共 54 個離線測試。adapter 尚未對 VPS 驗證，仍需真實 CLI/API/job 與 VPS E2E；依 owner 指示，這些整合驗收排在本機開發之後。[前置紀錄](M3-APP-DESIRED-STATE-PREP-2026-09-25.md)。
- ERU-013 本機功能進行中：patch validator 支援選定 patch、revision、精確 Go 版本／archive SHA 與版本專屬 test package；新增雙次 private build manifest publisher 與 versioned validator，綁定 source tag／commit、patch SHA、architecture、toolchain SHA、獨立重建 artifact SHA 及相容版本測試。core patch plan 區分 baseline install、同 artifact reapply、同版本 patch revision、跨版本 upgrade；隱性 downgrade 拒絕，回退仍必須引用原 source run。現有 v0.1.5 patch 已以 Go 1.27.1 隔離重驗：baseline regression tests 重現 panic，patch 後 calcium／locks tests 通過，兩次 clean archive build 與 validator build 都重現原 artifact SHA；沒有改 manifest 或部署。另對未發布的 upstream master `e9b48c12663f18e1356ee29f7ce2ac0963a8ae4d` 做選定 regression／calcium+store／locks tests 與 build，兩次獨立 build SHA 一致；這是前向相容性探測，不是穩定版發布或跨版本升級證據。尚無更新的 stable tag／正式 manifest，亦未做 VPS upgrade／rollback E2E。[provenance 紀錄](M3-CORE-RELEASE-PROVENANCE-2026-09-25.md)、[Go 1.27.1 重驗紀錄](M3-GO127-REVALIDATION-2026-09-25.md)、[master candidate probe](M3-CORE-MASTER-CANDIDATE-2026-09-26.md)。
- ERU-014 本機前置進行中：人工控制台 intent／receipt、replacement identity gate、hash-bound preparation／planner、worker-only install、registration 前 core known_hosts／nft access preparation、fenced registration、fenced smoke、safe-resume 與 resume 後 generation commit executors／唯讀 reconcile 均以 fake operator 驗證。generation commit 重驗 successful resume、worker IP／OOB host key 與 core access proof，再只更新 private deployment plan 的 target IP／core known_hosts／firewall render，最後將 generation 加一；journal/reconcile 可辨識精確 partial write 並要求明確續跑。這些測試只用 temp fake fixtures，沒有讀寫真實 private data。各階段不確定時先唯讀 reconcile，不重播遠端 mutation。safe AddNode patch：pinned v0.1.5 新 node 預設 Bypass=true，Go 1.27.1 兩個隔離 build 的 regression／calcium+store／lock tests／build 均通過。patch 仍 verified-not-deployed，未連 VPS。跨階段 recovery coordinator 已加入並以 fake tests 驗證；完整本機 suite（含 ERU-009 的 10 個 planner tests）於 2026-09-26 共 362 tests 通過。仍缺 safe patch 受控部署與 E2E，總剩餘數仍為 12。詳見 [safe AddNode](M3-CORE-SAFE-NODE-ADD-2026-09-26.md)、[core access](M3-REIMAGE-WORKER-ACCESS-2026-09-26.md)、[registration](M3-REIMAGE-WORKER-REGISTER-2026-09-26.md)、[smoke](M3-REIMAGE-WORKER-SMOKE-2026-09-26.md)、[resume](M3-REIMAGE-WORKER-RESUME-2026-09-26.md)、[generation commit](M3-REIMAGE-WORKER-GENERATION-2026-09-26.md) 與 [跨階段 recovery](M3-REIMAGE-WORKER-RECOVERY-2026-09-26.md)。
