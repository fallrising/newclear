...
更新：2026-09-26。固定剩餘任務編號以 TASKS.md 為準：仍為 12 項（近期 1、後續 11）；ERU-001～006 已完成，ERU-007 待正式 V11 小流量驗收。依 owner 指示先完成本機開發，再統一進行 VPS E2E。ERU-012 本機 desired-state 功能已齊待 E2E；ERU-013 provenance／版本 guard 已合併，v0.1.5 patch 已用 Go 1.27.1 隔離重驗，仍缺新 upstream 真實相容性證據；ERU-014 已加入重灌前 preparation、離線 worker bootstrap planner、worker-only install executor 與 read-only reconcile，均只用 fake operator／remote responses 驗證，未連 VPS，且安裝後 agent 停止、node 未註冊。重新納管、smoke、resume、恢復器仍待開發，任務數未減。每完成一項須更新清單並回報完成編號、剩餘數與下一項。24h 低頻觀測已於 2026-09-24 11:25:06 UTC 結束，摘要見 TODO-SOAK-2026-09-23.md。
...
- ERU-014 本機前置進行中：人工控制台 intent／receipt 綁定 owner-reviewed resource、OS image、volume scope、host identity 與 OOB host key；hash-bound preparation 只 fence／停止目標 agent／移除精確 node registration；離線 planner 產生 worker-only payload。新增 install executor 會再次核對來源、信任、主機身份、core health／membership、其他主機、乾淨安裝範圍與保留服務，只安裝鎖定 worker artifacts／六個 ERU 設定檔和與本機 pinned verified-host-public-keys.json 相符的 core Ed25519 key；authorized_keys 保留來源 IP、forced command 及 no-forwarding 限制。只啟動 proxy socket，不啟動 agent、不註冊 node。回覆不確定後只有 target/core read-only reconcile，不重播。58 項 operator 測試通過；沒有連 VPS、實機 intent／receipt、OS reimage 或安裝。仍缺重新納管、HTTP smoke、resume、恢復 executor 與 E2E；總剩餘數仍為 12。詳見 M3-REIMAGE-WORKER-INSTALL-2026-09-26.md。
