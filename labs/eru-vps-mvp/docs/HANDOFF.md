...
更新：2026-09-26。固定剩餘任務編號以 TASKS.md 為準：仍為 12 項（近期 1、後續 11）；ERU-001～006 已完成，ERU-007 待正式 V11 小流量驗收。依 owner 指示先完成本機開發，再統一進行 VPS E2E。ERU-012 本機 desired-state 功能已齊待 E2E；ERU-013 provenance／版本 guard 已合併，既有 v0.1.5 patch 已以 Go 1.27.1 隔離重驗；ERU-014 已有 worker-only install 與 fenced registration executors／唯讀 reconcile，並完成安全 AddNode patch 的 Go 1.27.1 雙環境建置和 resume helper 本機測試。patch 尚未部署；smoke、resume、generation commit、跨階段 recovery 與 E2E 仍待完成，任務數未減。每完成正式任務須更新清單並回報編號與剩餘數。24h 低頻觀測已於 2026-09-24 11:25:06 UTC 結束，摘要見 TODO-SOAK-2026-09-23.md。
...
- ERU-014 本機前置進行中：人工控制台 intent／receipt、replacement identity gate、hash-bound preparation／planner、worker-only install 與 fenced registration executors／唯讀 reconcile 均以 fake operator 驗證。registration 核對 core running binary SHA，只 AddNode 為 bypass=true、啟動 agent 並等待 available=true，停在 `registered-awaiting-smoke`；AddNode／agent start 回覆遺失時 reconcile 只讀、不重播。safe AddNode patch：pinned v0.1.5 新 node 預設 Bypass=true，Go 1.27.1 兩個隔離 build 的 regression／calcium+store／lock tests／build 均通過；resume helper 有 6 項離線測試。patch 仍 verified-not-deployed，未連 VPS 或修改 private inventory。仍缺受控 core 部署、smoke／其他 worker guards、resume、generation commit、跨階段 recovery 與 E2E；總剩餘數仍為 12。詳見 [safe AddNode](M3-CORE-SAFE-NODE-ADD-2026-09-26.md) 與 [registration](M3-REIMAGE-WORKER-REGISTER-2026-09-26.md)。
