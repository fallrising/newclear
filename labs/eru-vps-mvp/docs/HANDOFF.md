...
- ERU-014 本機前置進行中：人工控制台 intent／receipt、replacement identity gate、hash-bound preparation／planner、worker-only install、fenced registration 與 fenced smoke executors／唯讀 reconcile 均以 fake operator 驗證。registration 核對 core running binary SHA，只 AddNode 為 bypass=true、啟動 agent 並等待 available=true；smoke 在 peer canaries 的連續 HTTP guards 下完成 target nginx lifecycle checks 並清理，成功停在 `smoked-awaiting-resume`。AddNode／agent start／smoke response 不確定時先唯讀 reconcile，不重播。safe AddNode patch：pinned v0.1.5 新 node 預設 Bypass=true，Go 1.27.1 兩個隔離 build 的 regression／calcium+store／lock tests／build 均通過；resume helper 有 6 項離線測試。patch 仍 verified-not-deployed，未連 VPS 或修改 private inventory。仍缺受控 core 部署、safe resume、generation commit、跨階段 recovery 與 E2E；總剩餘數仍為 12。詳見 [safe AddNode](M3-CORE-SAFE-NODE-ADD-2026-09-26.md)、[registration](M3-REIMAGE-WORKER-REGISTER-2026-09-26.md) 與 [smoke](M3-REIMAGE-WORKER-SMOKE-2026-09-26.md)。
