...
後續新增的 registration executor 另有 fake-operator 測試，詳見 [重新納管階段](M3-REIMAGE-WORKER-REGISTER-2026-09-26.md)。它要求執行中 safe core hash，單次 AddNode、啟動 agent 並在 bypass=true 時等候 available=true；lost response 只允許 read-only reconcile。剩下的本機工作是目標 smoke／其他 worker guards、接上單次 safe resume、host-key 更新、generation commit 與跨階段恢復 executor。safe AddNode core artifact 尚未部署；全程未做 E2E。provider API 不使用；OS reimage 與日常 component-reinstall 分開驗收。
