...
結果寫入 private `reimage-bootstrap-plans/`，保存 source plan、receipt、preparation、observation 與 artifact lock hashes。CLI 摘要不輸出新 endpoint。總 plan 仍為 `executable: false`，install、registration、fenced smoke 與 safe resume 各有獨立 stage；剩餘 blocker 是 worker IP／host-key 對帳、generation commit 與 recovery executor。
