...
| ERU-009 | 非空 worker 的計畫性 drain／重裝／恢復 | 進行中 | [離線 planner、staged executor 與 recovery](M3-WORKER-NONEMPTY-DRAIN-PREP-2026-09-26.md)：planner 產生 ERU-012 完整、owned、stateless revisions 的 review-only 計畫；staged executor 綁定 live preflight、one-shot fence，全部 replacements exact 且 HTTP-ready 後才以 `AppRevisionCleanup` 按 exact IDs 清來源。唯讀 recovery 不重播 fence／deploy／remove；partial cleanup 後 reconcile parent 與 fresh child journals，只為仍存在的 exact source IDs 建一個 app 的新 hash-bound cleanup plan；target metadata、containers、tasks、usage 全零才開一般重裝 gate。`labctl` 提供離線 plan、唯讀 prepare、明確 execute、唯讀 recover，以及 fresh cleanup plan／execute 六個命令；25 個 drain fake tests、ERU-012 cleanup subset regression 與完整 378-test suite 通過（前一切片 375）。遠端命令／API/job 語意與整體 VPS E2E 尚未驗收，任務未完成。 |
