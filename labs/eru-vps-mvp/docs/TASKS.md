...
**目前剩餘 12 項：近期收尾 1 項，後續驗證／擴充 11 項。此清單內完成 6 項。** 依 owner 指示先完成本機開發，再統一做正式 VPS E2E。ERU-012 本機功能已齊待 E2E；ERU-013 的既有 v0.1.5 patch 已用 Go 1.27.1 隔離重驗，仍缺新 upstream 真實相容性證據；ERU-014 已加入 preparation、worker-only install executor 與唯讀 reconcile，並完成 pinned core 安全 AddNode patch 的雙環境離線建置；該 patch 尚未部署，重新納管／smoke／resume／恢復 executor／E2E 未完成，任務數不變。
...
| ERU-014 | 單 worker 人工 OS 重灌與重新納管 | 進行中 | Owner-reviewed intent／receipt、hash-bound preparation、replacement host verification、離線 worker plan、worker-only install executor 與 read-only reconcile 已以 fake operator 驗證，安裝後 agent 停止且 node 未註冊。另發現 pinned core v0.1.5 的 AddNode 預設未 fencing，新增 Bypass-at-Add patch、Go 1.27.1 雙環境建置及 resume helper 離線測試；安全 patch 為 verified-not-deployed，未連 VPS，helper 尚未接入 ERU-014 registration executor。仍缺 patch 受控部署、重新納管、smoke／其他 worker guards、resume、generation commit、分階段恢復 executor 與整體 E2E。provider API 不使用，OS reimage 與日常元件重裝分開驗收；見 M3-CORE-SAFE-NODE-ADD-2026-09-26.md。 |
