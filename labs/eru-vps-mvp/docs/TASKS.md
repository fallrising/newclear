...
**目前剩餘 12 項：近期收尾 1 項，後續驗證／擴充 11 項。此清單內完成 6 項。** 依 owner 指示，先收尾本機開發，再統一做正式 VPS E2E；ERU-012 本機功能已齊、待 E2E；ERU-013 provenance／版本 guard 已合併但缺新 upstream 真實相容性證據；ERU-014 正在完成 owner-reviewed 重灌 intent 前置，因此數量不變。後續項目保留原 SDD 範圍，不代表立即對 VPS 執行所有變更。
...
| ERU-014 | 單 worker 人工 OS 重灌與重新納管 | 進行中 | [離線 intent 前置](M3-REIMAGE-INTENT-2026-09-25.md) 已加入 private owner-reviewed 資源／OS image／精確 volume scope、現有 machine ID 綁定與 SHA-256；有效 plan 仍不可執行，缺 drain／重新納管／resume adapter 及新 host key／bootstrap。之後由 owner 到 provider 控制台操作，再核對 OS／新身分、SSH／Tailscale／runtime／ERU worker-only bootstrap、註冊與 smoke。與元件重裝分開驗收，不接 provider API。 |
