...
| ERU-009 | 非空 worker 的計畫性 drain／重裝／恢復 | 進行中 | [離線 planner 與 executor 前置](M3-WORKER-NONEMPTY-DRAIN-PREP-2026-09-26.md)：planner 產生 ERU-012 完整、owned、stateless revisions 的 review-only 計畫；staged executor 先綁 live preflight，再 one-shot fence target，逐 app 使用既有 `AppExecutor`，全部 replacements exact 且 HTTP-ready 後才使用 `AppRevisionCleanup` 按 exact IDs 清來源。read-only reconcile/recover 不重播 fence／deploy／remove；target metadata、containers、tasks、usage 全零才開一般重裝 gate。19 個 drain fake tests 與完整 371-test suite 通過（基準 362）；partial cleanup 仍只對帳、不自動續清理，尚缺 `labctl` 操作入口與真實 CLI/API/job／VPS E2E，任務未完成。 |
