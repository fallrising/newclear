...
**目前剩餘 12 項：近期收尾 1 項，後續驗證／擴充 11 項。此清單內完成 6 項。** 依 owner 指示，先收尾本機開發，再統一做正式 VPS E2E；ERU-012 本機功能已齊、待 E2E；ERU-013 現有 v0.1.5 patch 已依 Go 1.27.1 隔離重驗並重現 artifact hash，但缺新 upstream 真實相容性證據；ERU-014 已加入 owner-reviewed intent／receipt、唯讀新 host gate、worker-only payload builder／重灌後離線 bootstrap planner，以及需 hash-bound review 才可呼叫的重灌前 fence／agent stop／node deregistration 階段；新 host worker-only 安裝／重新納管與 E2E 未完成，因此數量不變。後續項目保留原 SDD 範圍，不代表立即對 VPS 執行所有變更。
...
| ERU-014 | 單 worker 人工 OS 重灌與重新納管 | 進行中 | [離線 intent／receipt 前置](M3-REIMAGE-INTENT-2026-09-25.md) 綁定 owner-reviewed 資源／OS image／精確磁碟 scope、舊／新機器與 boot identity、plan hash、帶外 SSH fingerprint；[重灌前摘除器](M3-REIMAGE-PREPARE-2026-09-26.md) 新增 hash-bound preparation gate，fake 測試 fence、只停 target `eru-agent`、保留 SSH/Tailscale/Docker/containerd、runtime recheck、精確 node deregistration、lost-response 不重播與唯讀 reconcile，且 receipt 強制依賴成功 preparation journal。[重灌後 worker-only 計畫器](M3-REIMAGE-WORKER-PLAN-2026-09-26.md) 以 replacement observation 的新 tailnet IP 準備 agent/CNI payload，保留舊 node capacity／labels 並計算下一代 generation；plan 仍不可執行。唯讀新 host gate 僅 fake SSH runner 驗證，payload 只離線生成；本輪未連 VPS、未建立實機 intent／receipt、未重灌。plan 仍 `executable: false`，provider API 不使用；仍缺 worker-only 遠端安裝、新 node registration、HTTP smoke、resume 與恢復執行器。與日常元件重裝分開驗收。 |
