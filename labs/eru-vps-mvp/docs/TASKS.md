...
| ERU-009 | 非空 worker 的計畫性 drain／重裝／恢復 | 進行中 | [離線 planner](M3-WORKER-NONEMPTY-DRAIN-PREP-2026-09-26.md) 產生 ERU-012 完整、owned、stateless revisions 的 review-only 搬移計畫，綁定 explicit destination 與 exact source IDs／digest／hash；unknown、legacy、partial 或 stale revisions fail closed。10 個 fake tests 通過；planner 固定 review-only，尚缺多 app staged executor／reconcile、fence／cleanup 接線及 E2E。舊 revision 在全部目的地 ready 前保留。 |
