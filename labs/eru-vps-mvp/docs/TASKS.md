...
**目前剩餘 18 項：近期收尾 7 項，後續驗證／擴充 11 項。此清單內完成 0 項。** 先按近期收尾推進；後續項目保留原 SDD 範圍，不代表立即對 VPS 執行所有變更。

下一項可直接做：**ERU-001**，先完成本機實作與測試。ERU-002 的背景程序仍持續執行。近期 7 項完成表示這批收尾工作完成，不等同原 SDD 全部 V01–V11 或完整 HA 已通過。
...
| 編號 | 任務 | 狀態 | 前置／完成標準 |
| --- | --- | --- | --- |
| ERU-001 | core 更新於替換前中斷的取消／封存 | 待做 | 先在本機實作新的 source-bound 操作；核對 replace-intent 尚未成立、原檔未改變，保留 journal／備份／未知資料；驗證中斷與回覆遺失，且 reapply 不會被錯誤放行。本機測試、操作文件與交付完成；實機 core 故障另列 ERU-005。 |
| ERU-002 | 回收並判讀目前 24h 觀測 | 進行中 | run `20260923T112336Z-561e71e7`，2026-09-23 11:25:06 UTC 開始、2026-09-24 11:25:06 UTC 截止。回收三台完整 evidence，驗證完整性、功能／服務／延遲與末端 cluster 狀態，記錄結果及適用範圍。失敗只記錄失敗，不追認成功；後續修復／重測未完成前不關閉本項。 |
| ERU-003 | 精確清理本次 canaries 與配額對帳 | 待做 | ERU-002 的觀測／證據處理完成後，按原 canary run `20260923T112207Z-99e9508c` 建立新 cleanup plan，核對 owner/run/IDs，再驗證 workloads／配額歸零及服務保留。 |
| ERU-004 | etcd 慢同步的原因、影響與處理成本分析 | 待做 | 綜合歷史故障、ERU-002 的結果與官方資料，交付已證實／未證實原因、未處理的影響、修復選項與成本／優先順序。不足以歸因時列明缺失證據；若需額外修復實作，追加任務編號，不能宣稱根因已解決。 |
...
| ERU-006 | 補齊 host network 與管理埠隔離驗收 | 待做 | 對照部署結果與 V03，完成私網 host-network HTTP、從適當外部觀測點核對管理埠不可達，以及 CNI 路徑／port ownership 的 evidence。部署測試等 ERU-003 後安排，不將 SSH 可用當作公網隔離證據。 |
| ERU-007 | 正式 V11 小流量運行驗收 | 待做 | ERU-002 與本項分開。原 SDD 要求 1 req/s、24h、成功率 ≥99%、無 OOM／etcd alarm／磁碟使用 >80%；補齊採樣與判讀後，以獨立 VPS 背景 run 執行並保存結果。現有 30 秒取樣不能直接算本項完成。 |
