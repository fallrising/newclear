...
| ERU-012 | 應用差異部署與真實無狀態服務 | 進行中 | 已有 v1 spec／digest planner 與離線 hash-bound executor state machine；caller-supplied health／consistency preflight、正規化 snapshot、controller-local mutation lock、原子 journal、lost-reply read-only reconcile、HTTP readiness 與舊版保留均有 fake-adapter 測試。尚缺正式 Eru CLI／SSH adapter、live preflight/resource integration、exact-ID cleanup、真實 API/job 與 VPS E2E。詳見 [離線前置](M3-APP-DESIRED-STATE-PREP-2026-09-25.md)。 |
