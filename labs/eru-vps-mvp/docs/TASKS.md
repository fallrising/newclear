...
| ERU-012 | 應用差異部署與真實無狀態服務 | 進行中 | 已有本機 v1 stateless HTTP spec／digest 與 fail-closed 唯讀 diff planner；計畫不可執行。尚缺 capacity／health 綁定、hash/journal executor、lost-reply reconcile、ready-before-cleanup、真實 API/job 與 VPS E2E。詳見 [離線前置](M3-APP-DESIRED-STATE-PREP-2026-09-25.md)。 |
