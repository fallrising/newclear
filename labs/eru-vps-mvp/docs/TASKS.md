...
| ERU-014 | 單 worker 人工 OS 重灌與重新納管 | 進行中 | Owner-reviewed intent／receipt、hash-bound preparation、replacement host verification、離線 worker plan、worker-only install、fenced registration 與 fenced smoke executors／read-only reconcile 均以 fake operator 驗證。registration 核對 running core SHA，AddNode 後維持 bypass=true 並等候 agent available；smoke 在 peer HTTP guards 下清理 target nginx，成功停在 `smoked-awaiting-resume`。pinned core v0.1.5 Bypass-at-Add patch 已用 Go 1.27.1 雙環境建置，resume helper 有離線測試；patch 仍 verified-not-deployed。仍缺受控部署、safe resume／核對、host-key 更新、generation commit、跨階段恢復 executor 與整體 E2E。provider API 不使用，OS reimage 與日常元件重裝分開驗收；見 [safe AddNode](M3-CORE-SAFE-NODE-ADD-2026-09-26.md)、[registration](M3-REIMAGE-WORKER-REGISTER-2026-09-26.md) 與 [smoke](M3-REIMAGE-WORKER-SMOKE-2026-09-26.md)。 |
