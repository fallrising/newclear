...
**目前剩餘 12 項：近期收尾 1 項，後續驗證／擴充 11 項。此清單內完成 6 項。** 依 owner 指示先完成本機開發，再統一做正式 VPS E2E。ERU-012 本機功能已齊待 E2E；ERU-013 現有 v0.1.5 patch 已用 Go 1.27.1 隔離重驗，但缺新 upstream 真實相容性證據；ERU-014 已加入重灌前 preparation、離線 bootstrap planner、worker-only install executor 與 read-only reconcile 的 fake 驗證，安裝階段保持 agent 停止且 node 未註冊；重新納管、smoke、resume、恢復 executor 與整體 E2E 未完成，任務數不變。
...
| ERU-014 | 單 worker 人工 OS 重灌與重新納管 | 進行中 | Owner-reviewed intent／receipt 綁定 provider resource、OS image、精確 volume scope、舊／新 machine／boot identity、plan hash 與帶外 SSH fingerprint。hash-bound preparation 以 fake operator 驗證 fence、只停 target agent、保留 SSH/Tailscale/Docker/containerd、runtime double-check、精確 registration removal、lost-response no-replay 與 read-only reconcile。重灌後 planner 綁定 strict-host observation 並生成 worker-only payload；新增 hash-bound install executor，安裝前重核 trust／身份／健康／membership／空 runtime／檔案 allowlist，只安裝鎖定 worker artifacts／設定及與 pinned verified-key input 相符的 core Ed25519 key，authorized_keys 使用來源限制與 forced command；僅啟動 proxy socket，保持 agent 停止且 node 未註冊。58 項 test_labctl.py 離線測試通過；失敗後只允許 read-only reconcile。未連 VPS、未建立實機 intent／receipt、未 OS reimage 或安裝。provider API 不使用，總 plan 仍不可執行；仍缺重新納管、HTTP smoke、resume、恢復 executor 與整體 E2E。與日常元件重裝分開驗收；見 M3-REIMAGE-WORKER-INSTALL-2026-09-26.md。 |
