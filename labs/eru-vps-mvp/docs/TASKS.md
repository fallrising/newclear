...
**目前剩餘 12 項：近期收尾 1 項，後續驗證／擴充 11 項。此清單內完成 6 項。** 依 owner 指示，先收尾本機開發，再統一做正式 VPS E2E；ERU-012 本機功能已齊、待 E2E；ERU-013 現有 v0.1.5 patch 已依 Go 1.27.1 隔離重驗並重現 artifact hash，但缺新 upstream 真實相容性證據；ERU-014 已加入 owner-reviewed intent／receipt、唯讀新 host gate、worker-only payload builder，以及需 hash-bound review 才可呼叫的重灌前 fence／agent stop／node deregistration 階段；新 host worker-only 安裝／重新納管與 E2E 未完成，因此數量不變。後續項目保留原 SDD 範圍，不代表立即對 VPS 執行所有變更。
