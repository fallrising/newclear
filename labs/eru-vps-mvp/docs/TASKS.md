...
**目前剩餘 12 項：近期收尾 1 項，後續驗證／擴充 11 項。此清單內完成 6 項。** 依 owner 指示，先收尾本機開發，再統一做正式 VPS E2E；ERU-012 本機功能已齊、待 E2E；ERU-013 現有 v0.1.5 patch 已依 Go 1.27.1 隔離重驗並重現 artifact hash，但缺新 upstream 真實相容性證據；ERU-014 已加入 owner-reviewed intent／receipt、唯讀新 host gate 及 worker-only payload builder；遠端安裝／重新納管與 E2E 未完成，因此數量不變。後續項目保留原 SDD 範圍，不代表立即對 VPS 執行所有變更。
...
| ERU-014 | 單 worker 人工 OS 重灌與重新納管 | 進行中 | [離線 intent／receipt 前置](M3-REIMAGE-INTENT-2026-09-25.md) 綁定 owner-reviewed 資源／OS image／精確磁碟 scope、舊／新機器與 boot identity、plan hash、帶外 SSH fingerprint，並以 immutable 私有記錄保存 owner receipt；42 個 labctl 定向測試及 4 個 worker payload 測試覆蓋 identity／scope／hash／OOB fingerprint、既有 trust file fingerprint／SHA 綁定、唯讀新 host gate、worker-only agent/CNI payload scope 和無 remote mutation；SSH gate 僅以 fake runner 測試，payload 僅離線生成，尚未連線或安裝。全程不接 provider API，plan 仍不可執行，也不自動信任新 key 或重新納管。仍缺 drain／resume adapter、worker-only 遠端安裝與 node re-registration；實機驗收由 owner 控制台操作後另做。與元件重裝分開驗收。 |
