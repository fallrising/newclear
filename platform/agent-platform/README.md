...
**目前狀態：M0 真實 KVM gate 已通過；M2 的真實 Cocoon／OpenHands 整合、四 VM 並行、事件重連與 diff 驗收已通過。M3 的 worker 恢復／fencing、安全取消、工具審批與安全暫停／恢復切片已通過；已補強輸出密鑰防漏與 diff 完整性；guest 控制憑證隔離與固定節點 egress policy 已驗收，完整 AT-07 與 AT-11 模型預算仍待完成。**
