...
- Version：0.1.0
- Date：2026-09-21
- Status：設計基準已合併；M0 固定單節點／none-lane 真實 KVM gate 已通過；M2 真實 runtime／固定模擬模型驗收已通過，下一步 M3
...
本文件定義預計實作的契約，不是現成功能說明。`MUST` 為此平台的驗收要求；上游已提供的能力與尚待驗證的整合，分別在 [研究紀錄](docs/reference-selection.md) 與第 16 節列明。實作進度與證據見 [M0](docs/M0.md)／[M1](docs/M1.md)／[M2](docs/M2.md)；已執行的 Docker 檢查不等於全部契約通過。
...
- 不把日期、版本 pin、延遲目標或文件存在當作功能完成證據。M0 的固定配置已通過硬體 gate；M2 真實 runtime／固定模擬模型切片已通過，下一步為最早未完成的 M3。
