...
目前停止點：**AT-11-C2b2 的 HTTPS provider transport 與 profile verification 已實作，Python／PostgreSQL 驗收通過；real-KVM acceptance 尚未通過，分支未合併。** 沒有呼叫付費／外部 provider，也未使用主機既有 provider key；真實費用仍 unknown。設計與限制見 [AT-11-C2b2](M3-HTTPS-PROVIDER.md)，本次結果見 [evidence](evidence/m3-https-provider-2026-09-24.json)。下一視窗指示見 [NEXT-PROMPT.md](NEXT-PROMPT.md)。
...
- 私有輸出留在 `/tmp/apm3-at11c2b2-20260924`，不要公開 TLS key、mock key、model／SDK request 或 journal。此分支尚未 PR／merge；即使 CI 通過，也須保持 real-KVM gate 未完成的狀態。
