...
已知停止點：
- AT-11-C2b2 已實作 `openai-compatible-https-v1` 固定 HTTPS Chat Completions transport、file-backed `credential_ref`／CA 驗證及 profile-pinned verification (`none`／`commands`／`fixture-m2`)；migration 012 只放寬新 model reference，保留舊 fixture profiles。最新 `make platform-check` 通過 45 個 dependency-free unit tests、200 個 PostgreSQL／HTTP platform tests、Ruff lint／format。兩個需要平台依賴的新測試已移入 `tests_platform`；run `36008490179` 在程式碼 commit `e099c6e` 的 check／web／control-plane 全部通過，含 browser acceptance。之後文件更新 run `36009297291` 依使用者指示在 browser acceptance 前取消，web job 通過、check／control-plane cancelled。handoff-only commit `48af9eb` 跳過 CI，PR 最新 head 沒有 checks 報告；最後完整通過 run 仍是 `36008490179`。本機無 Node/npm。沒有付費／外部 provider call，也沒有使用既有 provider key。Draft PR #82 已建立。詳見 `M3-HTTPS-PROVIDER.md`、`CONTINUATION-STATE.md` 與 evidence。
...
- C2b2 最新本機 45 個 dependency-free unit tests 與 200 個 PostgreSQL／HTTP platform tests 通過；完整 run `36008490179` 全通過。最新文件 commit 的 workflow `36009297291` 在 browser E2E 前取消，PR checks 因此 unstable。依使用者指示，整體開發完成前不得再跑 E2E；取得正式 same-journal recovery 決策前不得重跑 KVM 或 merge。C2b1 的三個真實 KVM 案例與原 isolation 回歸（29 項 terminal 檢查）已通過；開始新工作時仍重新查 CI/evidence。
- Pause 與模型 I/O 競爭時，舊授權 completion 不交付；不能證實安全工具邊界就保持 pausing／容量，可取消或期限回收。不要把 SDK error 當 paused。

使用者已同意外部 API 依賴先以 mock 為主；不要要求現在提供 endpoint 或 key。C2b2 code 已完成，Draft PR #82 的程式碼 commit `e099c6e` 曾完整通過 CI（run `36008490179`），後續文件 commit 的 run `36009297291` 在 browser acceptance 前依使用者指示取消。**E2E 只在整體開發完成後執行一次；期間避免更新開啟中的 PR 觸發 workflow。** KVM 尚未通過且同 journal 有 quarantined allocation intent。Repo 內沒有適用無 handle intent 的受支援 recovery command/API；先向使用者取得正式、可稽核的 same-journal recovery 決策，不手改／刪 row、不換 state dir、不降低 generation、不走 direct driver。取得決策並完成整體開發後，才可用 `sg kvm` 啟動 node／connector、確認 explicit deny-all／zero-warm／empty node，集中跑最後的 `mock-https-complete` 和必要 isolation 回歸；所有必要 gates 完成前不得 merge。模型 snapshot 不固定費率；OpenAI Order Form、稅與價格更正可能使公開價格不是實際帳單。沒有可信帳戶價格及 token 上界就不能提供硬金額上限；unknown usage 不能當零。不要使用主機既有 provider key 作隱含授權。完整 AT-07／AT-11／M3 仍未完成；Claude／Gemini 原生 adapter、streaming、真實自然語言 coding、M4 artifact／export／backup／GC／production 尚未完成。
...
5. 已授權提交、推送、建立 PR；但 PR workflow 包含 E2E，因此整體開發完成前不要向開啟中的 PR push。最終整合後集中執行一次 E2E；必要 CI／KVM gates 全通過後才合併 main。一般步驟不用再問確認。
