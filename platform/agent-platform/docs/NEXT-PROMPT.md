...
- C2b2 最新本機 45 個 dependency-free unit tests 與 200 個 PostgreSQL／HTTP platform tests 通過；修正後 GitHub Web／check／control-plane CI 待回報。本機 Web 22 項無法執行（無 Node/npm）；C2b2 real-KVM gate 未通過，且同 journal 有 unresolved allocation intent，先按正式 same-journal recovery gate 處理。C2b1 的三個真實 KVM 案例與原 isolation 回歸（29 項 terminal 檢查）已通過；以最新 evidence／CI 為準。
