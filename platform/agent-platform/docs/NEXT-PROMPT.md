...
以及 SDD.md、適用 AGENTS.md、M3-FIXTURE-BUDGET.md、M3-GUEST-MODEL.md、M3-MODEL-PROXY.md、M3-EGRESS.md、M3-GUEST-ISOLATION.md 和交接引用的控制／recovery 文件。
...
- 新 migration 010；完整 drain 後更新 API／worker／connector。全部 real workers 設 MODEL_PROXY_CONFIG 指向 0600 fixture config 才啟用；空值的新 run 保留 legacy guest fixture。Active 模式不符時 fail closed，不自動換模型。Launcher／OCI 不需重建，guest helper 不能原地升級。
- M0 45、平台 182 測試通過；新 budget 三個真實 KVM 案例含成功結算、零 dispatch 截止、超界回報保留全額，另回歸 AT-11-B 原 13 案（含 29 項 terminal 隔離）。以最新 evidence／CI 為準。
...
- 本次 worktree /home/ckc/test/codex/newclear-agent-at11c
- 私有配置／模型 payload／logs /tmp/apm3-at11c-20260923，勿公開原文；前次 /tmp/apm3-guest-model-20260923 保留。
- 沿用 /tmp/apm3-egress-20260923/journal 與 fences、terminal、/tmp/apm2-20260922 Cocoon cache／runtime；全部舊交接目錄保留。
- 本次收尾 VM／claims 為零、132 筆 journal 對應 VM 全部停止，connector／sandboxd 已停、測試 Postgres 已移除；這是交接時證據，不能取代開始前重新核對。
