...
既有產品驗收見 [STATUS](docs/STATUS.md)。新增 [三工作區設計](docs/sdd/09-shared-workspaces.md) 將 RD、Ops、Admin 深化為同一領域的角色投影；W1 工作區入口、分組導航與三角色首頁已驗收合併。W2 共用資源、工作單與 Redis／Kafka 閉環已驗收合併（PR36，9162685）；W3 依完整 integration contract 完成實作、驗收並由 PR37 合併；W4 [契約](docs/W4-INTEGRATION-CONTRACT.md) 已由 PR50 驗收合併；W5 [契約](docs/W5-INTEGRATION-CONTRACT.md) revision 1 已固定，Demo User／Team 身分切片於 `ace27ff` 實作並通過局部檢查，其餘 W5 功能與總驗收尚未完成。固定版本及接受／合併結果見 STATUS 和 PLAN。
...
[09 共用模型](docs/sdd/09-shared-workspaces.md) 定義 REQ-WS-01～10、資源／子資源／服務綁定及共用工作單；[10 RD](docs/sdd/10-rd-workspace.md)、[11 Ops](docs/sdd/11-ops-workspace.md)、[12 Admin](docs/sdd/12-admin-workspace.md) 分別定義角色任務、頁面與操作。[13 能力地圖](docs/sdd/13-capability-map.md) 對照需求關鍵字並區分交付深度；[14 交付與驗收](docs/sdd/14-workspace-delivery.md) 定義 W1–W5、遷移及 AC-WS-01～18。

此擴充沿用 INV-01～08 與 Mock-first 邊界。W1／W2 實作依各自 integration contract 同步更新 API、權限、狀態機、遷移與受影響的 01–07；09–14 中尚未交付的能力仍是後續目標，不得以規格文字認定功能已存在。K8s/MySQL 等專業控制面與真實雲端執行的範圍，按能力地圖逐項區分。
