...
為修正時限風險，[PR69](https://github.com/fallrising/newclear/pull/69) 將 CI job 上限提高至60分鐘，artifact 名稱加入 run attempt。它已合併為 `616d67e`；實際合併後 [CI35956716320](https://github.com/fallrising/newclear/actions/runs/35956716320) 也通過426原生、100 Chromium、12 Firefox/WebKit、3效能、2隔離和全部靜態／建置檢查。[Artifact10792066235](https://github.com/fallrising/newclear/actions/runs/35956716320/artifacts/10792066235) 綁定 `616d67e`，初始 gzip JS 297,608 bytes、LCP 中位數1,336ms、查詢 p95 1.1ms、Mock HTTP p95 185.6ms。main `616d67e` 的 dim-gate component tree 為 `775bcb7`，workflow tree 為 `b8a65ac`。最新 main 已於2026-09-24重新核對為 `70a409a`；其後提交未改動 dim-gate、workflow 或 PLAN。T048/W5 closeout 已完成，owner 已釋放。

SDD第14章只列 W1–W5，沒有 W6 或 AC-WS-19 之後的已接受範圍。能力地圖 CAP-01–28 中仍標為「後續／待釐清」的項目尚未排優先級、規格或驗收；下一輪先由產品 owner 選定能力與使用流程，再建立 task／AC／integration contract。不要由能力名稱推斷執行權限或真實整合範圍。不部署、不操作真實身分、雲端或外部通知。詳見 [W5 validation](../../../.team/reports/dim-gate-w5-validation.md)、PLAN DG-D102 及 PR closeout。

以下 W5/W4 候選段落及「待驗收／待合併」措辭均為歷史 checkpoint；目前狀態以上述實際 Git／CI 與 DG-D102 為準。

歷史 W5 驗證候選（2026-09-24）：[草稿 PR61](https://github.com/fallrising/newclear/pull/61) head `03c15883abfedd39b4f9fe4fe893a8e17fc391a8` 是一行 W2 測試就緒等待；前一 `b35e6ed` 完整 Chromium 99/100，唯一 Kafka 案在示範資料初始化前讀取空快照。原案單跑 1/1、修正後 Redis/Kafka 2/2，426 原生及靜態／契約檢查通過。`03c` 完整 Chromium100、[精確 CI35943030312](https://github.com/fallrising/newclear/actions/runs/35943030312)、獨立 T049 attempt3、benchmark/isolation 尚待結果；同版 Firefox/WebKit smoke 12/12 已通過。main `3982f64` 的 ERU/Kith 更新未涉及 dim-gate／其 CI／PLAN，最終證據提交前須正常整合。**W5 未驗收／未合併。** 接續依 PLAN DG-D100、[W5 validation](../../../.team/reports/dim-gate-w5-validation.md)與實際 Git/CI；下段 `b35e6ed` 是歷史 checkpoint。

歷史 W5 驗證候選（2026-09-24）：[草稿 PR61](https://github.com/fallrising/newclear/pull/61) head `b35e6ed502f60e5291e4d27dd804f6f189c6832d` 僅在已審查產品 `7ab67ce` 上修正兩個 W1/W2 歷史瀏覽器遷移斷言，聚焦 Chromium2/2；獨立[T049 attempt2](../../../.team/reports/T-049-attempt-2.md)複核 delta、50/50 原生與型別，沒有新增來源缺陷。新版 426/426 原生、凍結安裝與靜態／契約／demo build 通過，完整 Chromium100及[精確 CI35940402377](https://github.com/fallrising/newclear/actions/runs/35940402377) 正在執行；benchmark/isolation 待跑。舊 `7ab67ce` 完整 Chromium 為 98/100，兩失敗都是要求 v4 的過時測試，CI 已取消，不作最終證據。**W5 尚未驗收／合併。** 接續核對 PLAN DG-D099、[W5 validation](../../../.team/reports/dim-gate-w5-validation.md)與實際 Git/CI；下段 `7ab67ce` 是歷史 checkpoint。

歷史 W5 固定候選（2026-09-24）：[草稿 PR61](https://github.com/fallrising/newclear/pull/61) head `7ab67ce5eb99967b820fa59df44c31808a40d631` 已完成 AC-WS-13/14 的身分、功能灰度、註冊路由與安全通知產品，並修正獨立審查 F1–F7。426/426 原生、凍結安裝、lint、typecheck、docs、contracts、CI/architecture、demo 建置及專案灰度聚焦 Chromium 通過；獨立[T049 報告](../../../.team/reports/T-049-attempt-1.md)已在固定提交複驗 97 項、關閉 F1–F7，無剩餘確認來源缺陷。完整 Chromium、Firefox/WebKit、效能、隔離與[精確產品 CI35937914805](https://github.com/fallrising/newclear/actions/runs/35937914805) 正在／尚待驗證，**W5 未驗收、PR 未合併**。接續先核對 PLAN DG-D098、實際 Git/CI 與[W5 contract](W5-INTEGRATION-CONTRACT.md) revision 2；舊 `ace27ff` 身分 checkpoint 與取消的中間 CI 只是歷史。不得用未完成 gate 宣稱驗收。
...
歷史 W4 產品 checkpoint（2026-09-23）：固定 `dd90ccb` 已在草稿 PR50；400/400 原生、92/92 Chromium、12/12 Firefox／WebKit、3/3 效能、2/2 隔離及原生檢查通過。獨立 [T044 attempt3](../../../.team/reports/T-044-attempt-3.md) 關閉 F1/F2，無剩餘已確認來源缺陷。首次隔離命令因漏設本機函式庫路徑而無法啟動 Chromium，補上既定環境後同來源 2/2 通過；保留原失敗紀錄。精確產品 [CI35909643363](https://github.com/fallrising/newclear/actions/runs/35909643363) 尚在執行，PR50 未驗收／未合併，W5 未開始。接手先核對 PLAN DG-D094、[W4 validation](../../../.team/reports/dim-gate-w4-validation.md)、CI 與 PR；下面的 W4 F1/F2／W3 checkpoint 是歷史紀錄。

歷史 W4 接續（2026-09-23）：獨立 [T044 attempt2](../../../.team/reports/T-044-attempt-2.md) 在產品 `460424b` 關閉 F1，另確認 F2：可變監控規格驗證先於授權和冪等重放，造成舊 receipt 重放失敗與隱藏指標資訊差異。主控已本地修正並增加 AlertRule／SLO、跨範圍回歸；400/400 原生及凍結安裝、原生檢查、demo 建置通過。PR50 前一 head CI35908469757 和本機未完成的 Chromium 已在 F2 後取消。**W4 未驗收，W5 未開始**；固定修正提交、T044 attempt3、完整瀏覽器／效能／隔離、最新 head CI 與授權正常合併／實際 closeout 待完成。下面 W4 F1 與 W3 段落均為歷史 checkpoint；以 PLAN DG-D093、[W4 validation](../../../.team/reports/dim-gate-w4-validation.md) 及 PR50 為準。
...
```text
請接手 dim-gate，先讀目前 checkout 的 `platform/dim-gate/docs/HANDOFF-WORKSPACES.md`，依適用 AGENTS、DEVELOPMENT_PROMPT／PROTOCOL、root `.team/PLAN.md`、STATUS、SDD 與 docs/sdd/09–14 重新對帳 Git／PR／CI／owner。W1–W5 已驗收合併；核對 PR61、PR69 與 DG-D102，不重做舊成果。SDD第14章沒有定義 W6；先依 capability map 整理仍標「後續／待釐清」的候選，取得產品 owner 對新能力與可驗收流程的選擇後，才建立新 task／AC／contract 和隔離 run。既有 branch/worktree 一律保留；不 force/main push、不部署、不操作真雲端／外部通知／真憑證。每個新里程碑依 gate 完成實作、完整驗證、獨立唯讀 review、最新 head CI、授權的正常合併與 actual closeout，並更新 PLAN、reports、STATUS、PR 與 handoff。
```
