...
這是 dim-gate 前端預期的 BFF contract，**不是宣稱 AWS、Aliyun、Prism 或 CI 工具原生已有相同 API**。v0.1 由 MSW 實現；future backend 需自己完成 adapter 與 server-side authentication。M0–M3 將目前里程碑契約實體化為 Zod DTO 與 OpenAPI 文件，避免手寫頁面與 mock 各自漂移。
