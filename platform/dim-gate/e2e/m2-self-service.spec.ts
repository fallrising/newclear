...
test('AC-12: configure failure releases capacity and retry preserves environment/CI identity with attempt + 1', async ({ page }) => {
  ...
  const collisionDialog = page.getByRole('dialog', { name: '手動納管配置項' })
  await collisionDialog.getByRole('combobox', { name: 'Provider' }).selectOption('aliyun')
  await collisionDialog.getByLabel('名稱').fill('attempted failed-job identity takeover')
  await collisionDialog.getByLabel('外部識別碼').fill(firstCi)
  await collisionDialog.getByRole('button', { name: '確認納管' }).click()
  await expect(collisionDialog.getByRole('alert')).toContainText('相同 canonical identity 的 CI 已存在或已由交付作業保留。')
  await collisionDialog.getByRole('button', { name: '取消' }).click()
