...
test('AC-WS-01/02: legal Admin scope and own-work views survive workspace switches and refresh', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: '使用者', exact: true }).selectOption('user-rd-data')
  await page.getByRole('combobox', { name: '角色', exact: true }).selectOption('rd')
  await page.getByLabel('Scope ID', { exact: true }).fill('project-store')
  await page.getByRole('button', { name: '新增授權', exact: true }).click()
  await expect(page.getByRole('row', { name: /user-rd-data.*RD.*project-store/ })).toBeVisible()
  const submit = async (name: string) => {
    await page.goto('rd/catalog')
    await page.getByRole('link', { name: '開始申請', exact: true }).click()
    ...
    await page.getByRole('textbox', { name: '用途', exact: true }).fill('W1 own-work and shared project verification')
    await page.getByRole('button', { name: '確認並提交申請', exact: true }).click()
    await expect(page.getByRole('heading', { name: `${name} · 待審核` })).toBeVisible()
    return new URL(page.url()).pathname.split('/').at(-1)!
  }
  await become(page, 'user-rd-data')
  const otherId = await submit('w1-other-requester')
  await become(page, 'user-rd-commerce')
  const ownId = await submit('w1-own-requester')
  await page.goto('rd?projectId=project-store')
  const work = page.getByRole('region', { name: '待處理申請與發布', exact: true })
  await expect(work).toContainText(ownId); await expect(work).toContainText(otherId)
  const before = await snapshot(page)
  await page.getByRole('combobox', { name: '首頁工作範圍' }).selectOption('mine')
  await expect(work).toContainText(ownId); await expect(work).not.toContainText(otherId)
  await work.getByRole('link', { name: /w1-own-requester/ }).click()
  await expect(page).toHaveURL(new RegExp(`/rd/requests/${ownId}$`))
  await page.goBack(); await page.reload()
  await expect(page.getByRole('combobox', { name: '首頁工作範圍' })).toHaveValue('mine')
  await expect(work).not.toContainText(otherId)
  await page.getByRole('combobox', { name: '首頁環境' }).selectOption('env-checkout-dev')
  await page.getByRole('combobox', { name: '工作區', exact: true }).selectOption('admin')
  await expect(page).toHaveURL(/admin\?projectId=project-store&environmentId=env-checkout-dev$/)
  await expect(page.getByRole('region', { name: 'admin 工作首頁' })).toBeVisible()
...
test('AC-WS-01/02: multi-grant diagnostics keep source workspace and canonical return context', async ({ page }) => {
  test.setTimeout(60_000)
  await page.goto('rd')
  await grantOps(page)
  await page.goto('rd?projectId=project-store&environmentId=env-checkout-dev')
  await page.getByRole('region', { name: 'rd 工作首頁' }).getByRole('link', { name: /checkout/ }).first().click()
  const environmentPath = new URL(page.url()).pathname
  await page.getByRole('row').filter({ hasText: 'ci-idc-redis-01' }).getByRole('link', { name: '在 Ops 查看' }).click()
  await expect(page.getByRole('combobox', { name: '工作區', exact: true })).toHaveValue('rd')
  const back = page.getByRole('link', { name: '← 返回RD 業務研發', exact: true })
  await expect(back).toHaveAttribute('href', environmentPath)
  // This canonical visible diagnostic route must retain the same source even though the user also has Ops grants.
  await page.goto('ops/topology?ciId=ci-idc-redis-01')
  await expect(page.getByRole('combobox', { name: '工作區', exact: true })).toHaveValue('rd')
  await expect(back).toHaveAttribute('href', environmentPath)
  await page.reload()
  await expect(back).toHaveAttribute('href', environmentPath)
  await back.click(); await expect(page).toHaveURL(new RegExp(`${environmentPath}$`))
  await page.getByRole('combobox', { name: '工作區', exact: true }).selectOption('ops')
  await expect(page.getByRole('region', { name: 'ops 工作首頁' })).toBeVisible()
  ...
  await dialog.getByLabel('來源版本').fill('w1-diagnostic-source')
  await dialog.getByRole('button', { name: '確認觸發', exact: true }).click()
  await expect(page).toHaveURL(/rd\/pipelines\/[^/?]+$/)
  await page.getByRole('combobox', { name: '模擬前進幅度' }).selectOption('6')
  await page.getByRole('button', { name: '前進模擬時鐘', exact: true }).click()
  await expect(page.getByRole('heading', { level: 1 })).toContainText('成功')
  await page.goto('guide')
  await page.getByRole('combobox', { name: '情境應用' }).selectOption('app-checkout')
  await page.getByRole('combobox', { name: '情境環境' }).selectOption('env-checkout-dev')
  await page.getByRole('button', { name: '注入發布後延遲', exact: true }).click()
  await expect(page.getByText('已注入三個一分鐘異常樣本；可前往觀測與事件詳情。')).toBeVisible()
  await page.getByRole('combobox', { name: '工作區', exact: true }).selectOption('ops')
  await page.getByRole('region', { name: '優先事件', exact: true }).getByRole('link', { name: /checkout/ }).click()
  await expect(page).toHaveURL(/ops\/incidents\/[^/?]+$/)
  const incidentPath = new URL(page.url()).pathname, before = await snapshot(page)
  await page.getByRole('link', { name: '查看事件觀測範圍', exact: true }).click()
  await expect(page).toHaveURL(/rd\/observability\?/)
  await expect(page.getByRole('combobox', { name: '工作區', exact: true })).toHaveValue('ops')
  await expect(page.getByRole('link', { name: '← 返回Ops 維運', exact: true })).toHaveAttribute('href', incidentPath)
  await page.reload()
  await expect(page.getByRole('combobox', { name: '工作區', exact: true })).toHaveValue('ops')
  await page.goBack()
