...
test('W2 AC-16: genuine W1 active Release and ProvisionJob migrate, reload and complete once through visible clock', async ({ page }, info) => {
  ...
  try {
    ...
    expect(migrated.seedVersion).toBe('dim-gate-w5-v1')
    ...
    for (const field of ['assignments', 'applications', 'environments', 'placements', 'requests', 'releases', 'pipelineRuns']) expect(migrated.entities[field]).toEqual(legacy.snapshot.entities[field])
    for (const field of ['users', 'teams']) expect(migrated.entities[field]).toEqual(legacy.snapshot.entities[field].map((row: object) => ({ ...row, source: 'seed' })))
    ...
    for (const collection of ['platformFeatures', 'platformRoutes', 'notificationSubscriptions', 'notificationAttempts']) expect(migrated.entities[collection]).toEqual([])
    expect(migrated.entities.channels.map((row: { id: string }) => row.id)).toEqual(['demo-rd', 'demo-ops'])
