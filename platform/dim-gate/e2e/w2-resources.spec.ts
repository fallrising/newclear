...
test('W2 AC-03/04/16: existing staging request → Redis approval → execution → canonical RD/Ops binding survives reload', async ({ page }, info) => {
  ...
  try {
    ...
    await page.goto('ops/requests?source=request')
    await page.getByRole('combobox', { name: '處理階段', exact: true }).selectOption('execution')
    await expect(page.getByRole('row').filter({ has: page.getByRole('link', { name: requestId, exact: true }) })).toContainText('approved')
    await page.getByRole('link', { name: requestId, exact: true }).click()
    ...
    await page.goto('ops/requests?source=change')
    await page.getByRole('combobox', { name: '處理階段', exact: true }).selectOption('execution')
    await expect(page.getByRole('row').filter({ has: page.getByRole('link', { name: id, exact: true }) })).toContainText('approved')
    await page.reload()
    await expect(page.getByRole('combobox', { name: '處理階段', exact: true })).toHaveValue('execution')
    await page.getByRole('link', { name: id, exact: true }).click()
