...
for (const operation of operations) {
  ...
  const implemented = ['M0', 'M1', 'M2', 'M3', 'M4', 'W2', 'W3', 'W4'].includes(operation.milestone)
  paths[path] ??= {}
...
const document = {
  openapi: '3.1.0',
  info: { title: 'dim-gate BFF and demo controls', version: '0.4.0-w4',
    description: `Generated from Zod. M0–M4 and W2–W4 provide ${operations.filter((operation) => ['M0', 'M1', 'M2', 'M3', 'M4', 'W2', 'W3', 'W4'].includes(operation.milestone)).length} executable operations. Cross-entity scope, provider attributes, capacity and state invariants are enforced by the domain and behavioral tests, not fully expressible in JSON Schema. No real cloud, telemetry collector, external notification or live authentication is provided.` },
...
// The browser needs route/status descriptors, not OpenAPI registration and query factories.
const runtimeDestination = new URL('../src/api/runtime-operations.ts', import.meta.url)
const runtimeDescriptors = operations.map(({ method, path, status, demo, milestone }) => ({ method, path, status, demo: Boolean(demo), milestone }))
const runtimeContent = `/** Generated by pnpm generate:contracts; edit the canonical operation registry. */
export const runtimeOperations: readonly { method: string; path: string; status: number; demo: boolean; milestone: string }[] = ${JSON.stringify(runtimeDescriptors, null, 2)}\n`
if (process.argv.includes('--check')) {
  if (readFileSync(runtimeDestination, 'utf8') !== runtimeContent) throw new Error('Runtime operation drift: run pnpm generate:contracts and commit both generated artifacts.')
} else writeFileSync(runtimeDestination, runtimeContent)
// The Mock adapter only needs success codes that differ from the HTTP 200 default.
// Keep the complete descriptor above for contract parity tests without loading it at boot.
const statusDestination = new URL('../src/api/runtime-statuses.ts', import.meta.url)
const nonDefaultStatuses = runtimeDescriptors.filter(operation => operation.status !== 200)
const statusGroups = new Map<string, typeof nonDefaultStatuses>()
for (const operation of nonDefaultStatuses) {
  const key = JSON.stringify([operation.method, operation.demo, operation.status])
  statusGroups.set(key, [...statusGroups.get(key) ?? [], operation])
}
const statusMatchers = [...statusGroups].map(([key, rows]) => {
  const [method, demo, status] = JSON.parse(key) as [string, boolean, number]
  const alternatives = rows.map(({ path }) => path.replace(/[.*+?^$()|[\]\\]/g, '\\$&').replace(/\{[^/]+\}/g, '[^/]+'))
  return `[${JSON.stringify(method)},${demo},${status},new RegExp(${JSON.stringify(`^(?:${alternatives.join('|')})$`)})]`
})
const statusContent = `/** Generated by pnpm generate:contracts; edit the canonical operation registry. */
export const runtimeStatuses: readonly (readonly [string, boolean, number, RegExp])[] = [${statusMatchers.join(',')}]\n`
if (process.argv.includes('--check')) {
  if (readFileSync(statusDestination, 'utf8') !== statusContent) throw new Error('Runtime status drift: run pnpm generate:contracts and commit all generated artifacts.')
} else writeFileSync(statusDestination, statusContent)
