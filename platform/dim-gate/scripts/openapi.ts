...
for (const operation of operations) {
  ...
  const implemented = ['M0', 'M1', 'M2', 'M3'].includes(operation.milestone)
  paths[path] ??= {}
  ...
  paths[path][operation.method] = {
    ...
    description: implemented ? `Implemented by the demo adapter through ${operation.milestone}.`
      : `Forward contract for ${operation.milestone}; unavailable in M3. Success below specifies the future result, not current behavior.`,
...
const document = {
  openapi: '3.1.0',
  info: { title: 'dim-gate BFF and demo controls', version: '0.1.0-m3',
    description: `Generated from Zod. M0–M3 provide ${operations.filter((operation) => ['M0', 'M1', 'M2', 'M3'].includes(operation.milestone)).length} executable operations; M4 operations remain explicitly planned. Cross-entity scope, provider attributes, capacity and state invariants are enforced by the domain and behavioral tests, not fully expressible in JSON Schema. No real cloud or live authentication is provided.` },
