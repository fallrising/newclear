...
for (const operation of operations) {
  ...
  const implemented = ['M0', 'M1', 'M2', 'M3', 'M4', 'W2', 'W3'].includes(operation.milestone)
  paths[path] ??= {}
...
const document = {
  openapi: '3.1.0',
  info: { title: 'dim-gate BFF and demo controls', version: '0.3.0-w3',
    description: `Generated from Zod. M0–M4 and W2–W3 provide ${operations.filter((operation) => ['M0', 'M1', 'M2', 'M3', 'M4', 'W2', 'W3'].includes(operation.milestone)).length} executable operations. Cross-entity scope, provider attributes, capacity and state invariants are enforced by the domain and behavioral tests, not fully expressible in JSON Schema. No real cloud or live authentication is provided.` },
