import { z } from 'zod'
import { monitoringNavigationItemSchema, notificationSchema, type Center } from '../../domain/schema-models'
...
export function createShellClient(request: ApiRequest) {
  return {
    getNavigation: (center: Center) => request(`/navigation?${new URLSearchParams({ center })}`, z.array(monitoringNavigationItemSchema)),
