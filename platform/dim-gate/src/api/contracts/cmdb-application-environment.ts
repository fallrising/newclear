...
export function registerCmdbContracts({ z, d, w, s, id, listQuery, read, command }: ContractContext) {
  ...
  read('/cis', 'listCIs', d.pageSchema(s.CIView), 'M0', listQuery({ provider: d.providerSchema.optional(), kind: d.ciKindSchema.optional(), projectId: id.optional(), environmentId: id.optional(), health: d.healthSchema.optional(), freshness: z.enum(['fresh', 'stale', 'unknown']).optional() }, ['id', 'name', 'updatedAt', 'provider', 'kind', 'health']))
