...
import { startDemo, recoverDemoStorage } from '../demo/browser'
import { App } from './App'
...
function StartupFailure({ error: initialError }: { error: unknown }) {
  ...
  const recover = async (choice: 'reset' | 'memory') => {
    setPending(true)
    try { await recoverDemoStorage(choice); root.render(<App />) }
...
void startDemo(import.meta.env.VITE_DATA_MODE).then(() => root.render(<App />)).catch((error: unknown) => root.render(<StartupFailure error={error} />))
