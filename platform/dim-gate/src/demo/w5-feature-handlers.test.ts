...
describe('W5 registered feature HTTP contract', () => {
  it('publishes only fixed capabilities and current-scope deterministic preview', async () => {
    ...
    try {
      ...
      expect(await client.api.getPlatformFeature(id)).toMatchObject({ id, status: 'draft', revision: 1 })
      ...
      expect(preview.items.find(row => row.userId === 'user-rd-commerce')).toMatchObject({ eligible: true, reason: 'active-cohort' })
