...
function activeTemplate(snapshot: Snapshot) {
  return snapshot.entities.notificationTemplates.find(row => row.status !== 'disabled' && row.activeRevision !== null)
}
...
export function dispatchNotification(snapshot: Snapshot, delivery: NotificationDelivery): NotificationAttempt[] {
  ...
  const template = activeTemplate(snapshot)
  ...
  const candidates = serviceTarget
    ...
    : [...demoPersonaIds].filter(id => {
      ...
      return !!policy.user && policy.centers.includes('ops') && policy.poolIds.includes(scope.poolId!)
