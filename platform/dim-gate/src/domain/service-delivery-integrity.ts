...
export function serviceDeliveryIntegrityErrors(s: Snapshot): string[] {
  ...
  for(const execution of e.serviceExecutions) {
    ...
    if(!execution.startedAt||Date.parse(execution.startedAt)>Date.parse(new Date(Date.parse('2026-09-20T09:00:00Z')+s.logicalClock*1000).toISOString()))error('invalid execution start')
    ...
    if(active) {
      ...
      if(execution.state==='failed'&&(!execution.failureCode||execution.stepResults.filter(step=>step.state==='failed').length!==1))error('missing execution failure evidence')
    }
...
function validateTraffic(execution:TrafficExecution,source:TrafficPolicy,s:Snapshot,error:(message:string)=>void) {
  ...
  for(const [i,sample] of execution.samples.entries()) {
    const step=execution.stepResults.find(step=>step.weight===sample.step),previous=execution.samples[i-1]
    if(seen.has(sample.id)||sample.executionId!==execution.id||sample.policyId!==source.id||sample.candidateReleaseId!==spec.targets[1].releaseId||Date.parse(sample.to)-Date.parse(sample.from)!==30_000
      ...
      ||previous&&Date.parse(previous.to)>Date.parse(sample.from))error('invalid traffic sample reference or interval')
  ...
  for(const [i,cp] of execution.checkpoints.entries()) {
    ...
    if(cp.step!==[10,50,100][i]||step.state!=='succeeded'||cp.sampleIds.length!==spec.healthWindowSeconds/30||new Set(cp.sampleIds).size!==cp.sampleIds.length
