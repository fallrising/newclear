...
describe('W5 registered feature cohort', () => {
  ...
  it('intersects current grant, team and deterministic cohort on domain commands', async () => {
    ...
    expect(current()).toMatchObject({ everActivated: false, activeRevision: null, status: 'draft' })
    ...
    await command('user-rd-commerce', '/monitor-policies', { ...originalBody, reason: 'New work during initial draft' })
    ...
    expect(current().everActivated).toBe(true)
    expect(featureEligibility(engine.getSnapshot(), policyFor(engine.getSnapshot(), 'user-rd-commerce'), 'rd.monitoring', 'project-store'))
  ...
  it('keeps active revision through draft, disables safely and restores as new immutable revision', async () => {
    ...
    expect(current()).toMatchObject({ status: 'draft', revision: 3, activeRevision: null, everActivated: true })
    expect(featureEligibility(engine.getSnapshot(), policyFor(engine.getSnapshot(), 'user-rd-commerce'), 'rd.monitoring', 'project-store'))
      .toMatchObject({ eligible: false, reason: 'inactive-policy' })
