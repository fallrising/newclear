...
import type { CatalogItem, Center, ModelField, NavigationItem, SessionView } from '../../domain/schemas'
...
export function AdminCatalogPage() {
  ...
  return <><PageHeading eyebrow="ADMIN · CATALOG" title="服務目錄治理" description="Published revision 不可直接改寫；先建立下一版 draft，發布後 RD 才能申請。" /><section className="panel admin-form"><label className="span-all">變更理由<input value={reason} onChange={(event) => setReason(event.target.value)} /></label></section>{catalog.isPending ? <LoadingState /> : catalog.isError ? <ErrorState error={catalog.error} /> : <div className="catalog-grid">{catalog.data.items.map((item) => <article className="panel" key={`${item.id}:${item.version}`}><div className="panel-title"><BookOpen size={20} /><h2>{item.name}</h2><span className="tag">rev {item.revision} · {statusLabel[item.status]}</span></div><p className="muted">{item.description}</p><dl className="detail-list"><div><dt>Projects</dt><dd>{item.allowedProjectIds.join(', ')}</dd></div><div><dt>Providers</dt><dd>{item.template.allowedProviders.join(', ')}</dd></div><div><dt>Limits</dt><dd>{item.template.limits.maxCpu} vCPU · {item.template.limits.maxMemoryMiB} MiB</dd></div></dl>{item.status === 'draft' && <CatalogDraftEditor item={item} committed={() => catalog.refetch()} />}<div className="request-actions">{item.status !== 'draft' && <Button variant="outline" disabled={mutation.isPending || !reason.trim()} onClick={() => void run('draft', item)}>建立下一版草稿</Button>}{item.status === 'draft' && <Button disabled={mutation.isPending || !reason.trim()} onClick={() => void run('publish', item)}>發布 revision</Button>}{item.status === 'published' && <Button variant="destructive" disabled={mutation.isPending || !reason.trim()} onClick={() => void run('disable', item)}>停用新申請</Button>}</div></article>)}</div>}<MutationFeedback mutation={mutation} success="服務目錄狀態已更新。" /></>
}
...
export function AuditPage() {
  ...
  const audit = useQuery({ queryKey: queryKey('audit', null, { actorId, entityType }), queryFn: () => api.listAudit({ ...(actorId ? { actorId } : {}), ...(entityType ? { entityType } : {}), page: 1, pageSize: 100, sort: 'occurredAt', order: 'desc' }) })
  return <><PageHeading eyebrow="ADMIN · AUDIT" title="管理稽核" description="安全 diff 摘要可依 actor/entity 篩選；不保存任意 payload、token 或完整 log。" /><form className="panel filter-bar" onSubmit={(event) => event.preventDefault()}><label>Actor ID<input value={actorId} onChange={(event) => setActorId(event.target.value)} /></label><label>Entity type<input value={entityType} onChange={(event) => setEntityType(event.target.value)} /></label></form>{audit.isPending ? <LoadingState /> : audit.isError ? <ErrorState error={audit.error} /> : audit.data.items.length === 0 ? <section className="panel empty-state"><h2>目前沒有符合條件的稽核</h2></section> : <div className="panel table-scroll"><table><caption>安全管理稽核；共 {audit.data.total} 筆</caption><thead><tr><th>時間 / action</th><th>Actor</th><th>Entity</th><th>Outcome</th><th>Diff / reason</th><th>Correlation</th></tr></thead><tbody>{audit.data.items.map((entry) => <tr key={entry.id}><th scope="row">{new Date(entry.occurredAt).toLocaleString('zh-TW')}<small>{entry.action}</small></th><td><code>{entry.actorId}</code></td><td>{entry.entityType}<small><code>{entry.entityId}</code></small></td><td>{entry.outcome}</td><td>{entry.diffSummary.join('、')}{entry.reason && <small>{entry.reason}</small>}</td><td><code>{entry.correlationId}</code></td></tr>)}</tbody></table></div>}</>
}
