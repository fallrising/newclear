...
export function AccessPage({ session }: { session: SessionView }) {
  ...
  return <><PageHeading eyebrow="ADMIN · ACCESS" title="角色與範圍" description="固定角色搭配 org/project/pool scope；不能修改自己，也不能移除最後一位啟用中的 Admin。" />
    <div className="request-actions"><Button asChild variant="outline"><Link to="/admin/users">管理 Demo 使用者與團隊</Link></Button></div>
...
export function AdminOverviewLinks() {
  return <section className="panel"><div className="panel-title"><Shield size={20} /><h2>治理工作區</h2></div><div className="admin-link-grid"><Button asChild variant="outline"><Link to="/admin/access">角色與範圍</Link></Button><Button asChild variant="outline"><Link to="/admin/features">功能灰度</Link></Button><Button asChild variant="outline"><Link to="/admin/navigation">導航目錄</Link></Button><Button asChild variant="outline"><Link to="/admin/catalog">服務目錄治理</Link></Button><Button asChild variant="outline"><Link to="/admin/cmdb-models">CMDB 自訂欄位</Link></Button><Button asChild variant="outline"><Link to="/admin/audit"><ScrollText size={16} />管理稽核</Link></Button></div></section>
}
