...
export function ApplicationDetailRoute({ client = defaultClient, canReadResources = false, canReadServiceDelivery = false, canReadAlerting = false }: { client?: ApplicationClient; canReadResources?: boolean; canReadServiceDelivery?: boolean; canReadAlerting?: boolean }) {
  ...
  return <>
    ...
    {canReadServiceDelivery && <ServiceDeliveryLinks appId={application.id} />}
    {canReadAlerting && <AlertingLinks appId={application.id} />}
...
export function EnvironmentDetailRoute({ client = defaultClient, canReadResources = false, canReadServiceDelivery = false, canReadAlerting = false }: { client?: ApplicationClient; canReadResources?: boolean; canReadServiceDelivery?: boolean; canReadAlerting?: boolean }) {
  ...
  return <>
    ...
    {canReadServiceDelivery && <ServiceDeliveryLinks appId={appId} environmentId={environment.id} />}
    {canReadAlerting && <AlertingLinks appId={appId} environmentId={environment.id} />}
...
function ServiceDeliveryLinks({ appId, environmentId }: { appId: string; environmentId?: string }) {
  const suffix = environmentId ? `?${new URLSearchParams({ environmentId })}` : ''
  return <nav className="button-row" aria-label="服務交付與運行配置">
    <Button asChild variant="outline"><Link to={`/rd/apps/${encodeURIComponent(appId)}/delivery${suffix}`}>交付定義</Link></Button>
    <Button asChild variant="outline"><Link to={`/rd/apps/${encodeURIComponent(appId)}/configuration${suffix}`}>服務配置</Link></Button>
    <Button asChild variant="outline"><Link to={`/rd/apps/${encodeURIComponent(appId)}/traffic${suffix}`}>業務流量灰度</Link></Button>
  </nav>
}

function AlertingLinks({ appId, environmentId }: { appId: string; environmentId?: string }) {
  const suffix = environmentId ? `?${new URLSearchParams({ environmentId })}` : ''
  return <nav className="button-row" aria-label="服務監控與告警">
    <Button asChild variant="outline"><Link to={`/rd/apps/${encodeURIComponent(appId)}/monitoring${suffix}`}>監控設定</Link></Button>
    <Button asChild variant="outline"><Link to={`/rd/apps/${encodeURIComponent(appId)}/alerts${suffix}`}>告警規則</Link></Button>
  </nav>
}
