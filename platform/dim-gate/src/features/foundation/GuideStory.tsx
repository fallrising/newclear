...
export function GuideStory({ data, session }: { data: GuideView; session: SessionView }) {
  ...
  return <section className="panel guide-story">
    ...
    <section className="guide-preparation"><h2>Redis／Kafka 資源故事</h2><p>RD 從服務目錄提出資源需求；Ops 核對影響與配額，審批後另行啟動執行。共用時鐘完成驗證後，服務資源頁與專業頁會顯示相同的綁定及來源。執行失敗保留原始歷史，重新送審需要新決策。</p><div className="guide-scenario-actions">{session.centers.includes('rd') && <><Button asChild variant="outline"><Link to="/rd/catalog">提出資源需求</Link></Button><Button asChild variant="outline"><Link to="/rd/requests?source=change">追蹤我的資源工作單</Link></Button></>}{session.centers.includes('ops') && <><Button asChild variant="outline"><Link to="/ops/requests?source=change">處理資源審批</Link></Button><Button asChild variant="outline"><Link to="/ops/caches">查看 Redis 共享資源</Link></Button><Button asChild variant="outline"><Link to="/ops/messaging">查看 Kafka 資源</Link></Button><Button asChild variant="outline"><Link to="/ops/clusters">查看 Kubernetes 唯讀摘要</Link></Button></>}{session.centers.includes('admin') && <Button asChild variant="outline"><Link to="/admin/catalog">管理資源模板</Link></Button>}</div><p className="muted">全部操作使用示範資料；這些入口不代表已完成流程，不連接真實資源。</p></section>
