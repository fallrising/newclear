...
export function Guide({ session }: { session: SessionView }) {
  ...
  const advanceClock = async () => {
    if (cache.isMutating({ mutationKey: demoClockMutationKey }) || cache.isMutating({ mutationKey: serviceDeliveryMutationKey })) return
    setClockNotice('')
  ...
  return <>
    ...
    <div className="guide-layout">
      ...
      <section className="panel session-panel"><div className="panel-title"><Clock3 size={20} aria-hidden="true" /><h2>Session 控制</h2></div>{guide.isPending ? <LoadingState label="正在讀取演示進度…" /> : guide.isError ? <ErrorState error={guide.error} onRetry={() => void guide.refetch()} /> : <>
        ...
        <form className="clock-form" onSubmit={(event) => { event.preventDefault(); void advanceClock() }}><label htmlFor="clock-ticks">前進幅度</label><div><select id="clock-ticks" value={ticks} disabled={clockPending || serviceCommandPending} onChange={(event) => setTicks(Number(event.target.value))}><option value={1}>1 tick</option><option value={5}>5 ticks</option><option value={10}>10 ticks</option><option value={60}>60 ticks</option></select><Button type="submit" disabled={clockPending || serviceCommandPending || reset.isPending}><Clock3 size={16} aria-hidden="true" />{clockPending ? '正在前進…' : '前進演示時鐘'}</Button></div></form>
