...
import { actionLabel, changeKindLabel, changeStateLabel, dateLabel, isMissing, MissingResource, QuotaView, purposeLabel } from './shared'
...
export function ChangeDetailPage({ center, session }: { center: 'rd' | 'ops'; session: SessionView }) {
  ...
  return <>
    ...
    <section className="panel"><h2>影響與配額</h2>{data.impact.incomplete?<p role="status">影響範圍未完整授權，不能宣稱已完整核對。</p>:data.impact.consumers.map(consumer=><p key={consumer.environmentId}>{consumer.applicationName} / {consumer.environmentName} · {consumer.stage}</p>)}<QuotaView capacity={data.capacity}/></section>
