...
export function WorkItemsPage({ center, session }: { center: 'rd' | 'ops'; session: SessionView }) {
  ...
  return <>
    ...
    {work.isPending ? <LoadingState label="正在讀取授權範圍內的工作單…" /> : work.isError ? <ErrorState error={work.error} onRetry={() => void work.refetch()} /> : work.data.items.length === 0 ? <section className="panel empty-state" role="status"><h2>目前沒有符合條件的工作單</h2><p>只計算 {session.user.displayName} 目前被授權且符合篩選的資料。</p></section> : <section className="panel"><div className="table-scroll"><table><caption>共 {work.data.total} 筆工作單；第 {page} 頁</caption><thead><tr><th scope="col">來源／編號</th><th scope="col">服務與目標</th><th scope="col">申請／批准</th><th scope="col">狀態</th><th scope="col">時間</th></tr></thead><tbody>{work.data.items.map(item => <tr key={`${item.sourceType}:${item.sourceId}`}><th scope="row"><span className="tag">{sourceLabel[item.sourceType]}</span><Link to={item.route}>{item.sourceId}</Link></th><td>{item.targetRefs.map(ref => <small key={`${ref.entityType}:${ref.entityId}`}><code>{ref.entityId}</code></small>)}</td><td>{item.requester.displayName}<small>{item.approver ? `決策：${item.approver.displayName}` : '尚無批准者'}</small></td><td>{item.stateLabel}<small><code>{item.rawState}</code> · {item.actionRequired ? '可處理' : '追蹤中'}</small></td><td>{dateLabel(item.updatedAt)}<small>來源經過 {Math.max(0, Math.floor((Date.parse(item.dataAsOf) - Date.parse(item.createdAt)) / 1000))} 秒；示範邏輯時鐘</small></td></tr>)}</tbody></table></div>
