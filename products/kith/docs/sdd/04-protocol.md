...
| GET | `/api/rooms/:id/members` | member | 本房成員、kind、attention、quota_class、operator-only badge；`reply_limit` 見 [10](10-members-and-mention.md)；畫面見 [11](11-room-screen.md) |
| POST | `/api/rooms/:id/members` | **owner** | 加入既有成員。Body 恰好帶 `member_id` 或 `handle` 其中一個，可加 `role`（預設 `member`）。兩條路徑都含未停用的 human 與 agent（handle 為 `COLLATE NOCASE`）。找不到、或帳號已停用 → 404 `not_found`。兩個識別都有、或都沒有 → 400 `invalid_request`。滿 32 人 → 409 `room_full`。handle 亦解析未停用 agent；`reply_limit` 見 [10](10-members-and-mention.md)；畫面見 [11](11-room-screen.md) |
