...
- 禁止：任何聊天功能；修改後端。
- 狀態：`VERIFIED`（施工圖：[milestones/W0.md](milestones/W0.md)；實作 PR #72，全套 run `20260924-065045-dc89e25`：`E2E-W0-01` desktop／mobile、`E2E-W0-02` 通過，`e2e:validate` ok）。
...
- 狀態：`VERIFIED`（施工圖：[milestones/W1.md](milestones/W1.md)；全套 run `20260924-071613-24d2cc8`：W0–W1 共 9 個測試通過，`e2e:validate` ok）。
...
- 狀態：`IN_PROGRESS`（施工圖：[milestones/W6.md](milestones/W6.md)）。`kith-runner` 在 repo 內 `runner/`，`sidecar/` 不改；契約 [mcp-events.json](../../contracts/v2/mcp-events.json)、[http-traces.json](../../contracts/v2/http-traces.json)、[runner-config.json](../../contracts/v2/runner-config.json)。
