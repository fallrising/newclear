...
| ID | 來源 | 要確認的事 | 狀態 |
| --- | --- | --- | --- |
| S2-01 | OpenAI API reference：Chat Completions、Responses、Models | 請求／回應欄位、串流事件、錯誤碼、`retry-after` | 部分查證 2026-09-24（Chat Completions、Models、錯誤 envelope）：`openai` npm 7.23.0 原始碼與 [openai-openapi](https://raw.githubusercontent.com/openai/openai-openapi/master/openapi.yaml)，見 [W4](milestones/W4.md) §4.5.1。Responses 與串流事件在 W5 |
| S2-02 | Anthropic API reference：Messages、Models、Streaming、Errors | `anthropic-version` 現行值、SSE 事件類型、529 | 部分查證 2026-09-24：`@anthropic-ai/sdk` npm 0.128.0 原始碼（`anthropic-version: 2023-06-01`、`StopReason`、`ErrorType` 含 `overloaded_error`、SSE 事件名稱），見 [W4](milestones/W4.md) §4.5.2。串流的逐欄對照在 W5 |
...
- [ ] S2 各來源查證，附 URL 與查閱日期，並把結論寫回 03、04、07、08。S2-08、S2-09 已完成（W0）；新增 S2-10、S2-11（W0）；S2-01、S2-02、S2-04、S2-05、S2-07 的 W4 部分完成（W4 §4.5）；S2-03 與串流在 W5，S2-06 在 W6。
