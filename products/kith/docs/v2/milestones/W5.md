...
- 狀態：`VERIFIED`（2026-09-26，全套 run `20260926-070300-349aff0`）
...
四種方言、串流、中途斷線。新增指令：`chunks`（把回覆切成幾段，1–200，預設 4）、`chunk_ms`（段與段的間隔，0–5000，預設 150）、`cut_after`（送出幾段後直接關閉 socket，不送任何結束事件）。log 多一個欄位 `stream`。沒有 `delay_ms` 時仍等 500ms，讓 W4 的「正在回覆」佔位留得夠久；明確的 `delay_ms`（含 0）照原值。
...
- **目標**：`npm run e2e -- --grep @W5` exit 0；`manifest.json` 的 `redaction.hits` 為空。
- **輸入**：W5-T01–W5-T13。
- **驗證**：上述。
- **對應 ID**：E2E-W5-01–05。
- **預估大小**：S。
...
| 2 | 每 100 ms 讀一次 `reply-draft` 的 `textContent`，直到 `message-body` 文字等於 `T`（最多 15 s）。觸發列含編碼後的 `T`，也含 `rand`，不能拿來當停止條件 | — | 讀到的相異值 ≥ 3；每個值都是 `T` 的前綴且長度遞增；`note(info, "draft grew in N steps")` | `02-mid-draft`（第 2 個相異值時） |
...
| 5 | `const r = chaos.add({ kind: "hold", match: { dir: "in", type: "draft" } })`（同 W1 的 `WsChaos.add`）；ben 送 `@w5s_${rand} [[fake:text=late-${rand};chunks=4;chunk_ms=200]]` | ben 頁有一則 `message-body` 文字等於 `late-${rand}`（觸發列也含這段字，不能只看 `message-row`） | — | — |
...
| 4 | `provider-base-url` 改為 `FAKE().google`；名稱 `w5g-${rand}`；key、quota；`provider-test` → `provider-save` | 測試結果模型為 `fake-gemini`、`fake-gemini-flash`；清單出現 `w5g-${rand}` 後才讀 id | — | `02-gemini-test` |
...
| 6 | 對每個 agent：ben 送 `@<handle> [[fake:text=${enc("fmt " + handle)};chunks=5;chunk_ms=300]]` | 出現文字 `fmt <handle>` 的 `message-row` | 串流的兩個：途中曾出現 `reply-draft`；非串流的兩個：從未出現 `reply-draft` | `03-<handle>`（`shot()` 不收 `_`，底線改成 `-`） |
...
- [x] 文件先行。
- [x] 只碰 §3 的檔案。游標動畫在 `web/src/styles/index.css`（應用實際載入的樣式，不是 `web/src/index.css`）。清單上的格式文字在 `ProvidersPage.tsx`。
- [x] `npm test`（含 `test/v2` 91 個）、`npm run lint`、兩個 lint exit 0。
- [x] `npm run e2e -- --grep @W5` 通過；`npm run e2e:all`（W0–W5）通過；`npm run e2e:validate` 印 `ok`。
- [x] `summary.md` 貼進 PR；`manifest.json` 的 `redaction.hits` 為空。
- [x] `wrangler.toml` 沒有打開 `ff_drafts`（正式環境在 W7 切換）。
- [x] 連結檢查通過。
- [x] [09](../09-roadmap.md) W5 狀態改為 `VERIFIED`。
