...
| 5 | `const r = chaos.add({ kind: "hold", match: { dir: "in", type: "draft" } })`（同 W1 的 `WsChaos.add`）；ben 送 `@w5s_${rand} [[fake:text=late-${rand};chunks=4;chunk_ms=200]]` | ben 頁出現 `late-${rand}` 的 `message-row` | — | — |
