...
- 狀態：`VERIFIED`（2026-09-26，全套 run `20260926-115611-1bcd56b`）
...
`ReplyState` 加 `phase: "replying" | "accepted" | "running"`（`is replying` 為 `replying`）。`ReplyPlaceholder` 的 `aria-label` 與可見標籤依 phase：`reply.replying`（W3）、`reply.accepted`、`reply.running`；三個點不變（runner 沒有草稿）。`accepted` 換成 `running` 前，佔位至少顯示 `reply.accepted` 300ms（runner 這兩次 `post_status` 往往只隔數十毫秒）。`ReplyFailed` 在 `blocked` 時顯示 `reply.blocked`（「{name} 沒有完成」）。
...
| 7 | ben 送 `@w6r_${rand} [[cli:exit=3;stderr=boom]]` | ben 頁 `reply-failed` 可見，文字 = `reply.blocked` | 主時間線出現「（@w6r_${rand} 沒有完成：exit 3）」；點該列 `thread-summary` 後等 `thread-panel` 的 `data-root` 換成這則，再展開 trace，其中一張摘要含 `boom`（FM-RUN-03） | `05-blocked` |
| 8 | ben 送 `@w6r_${rand} [[cli:big=1;traces=0]]` | 主時間線出現以 `（已截斷）` 結尾的訊息 | 該訊息 UTF-8 長度 = 8192；點該列 `thread-summary` 後等 `data-root` 換成這則，trace 全文長度 10,000（FM-RUN-04） | — |
...
| 3 | ben 在 `thread-composer-input` 輸入 `reply ${rand}`。桌面按 Enter；觸控（`pointer: coarse`，與 W1 `sendViaComposer` 相同）點 `thread-composer-send` | 面板的 `thread-replies` 出現 `reply ${rand}` | 主時間線沒有 `reply ${rand}`；`root ${rand}` 下方 `thread-summary` = 1 則 | `02-replied` |
...
**test 2**：`"E2E-W6-04 FM-RUN-02/05 gap and duplicate live events against a stub Kith"`（標題需 W0 的驗收 id）；不開瀏覽器。
...
- [x] 文件先行。
- [x] 只碰 §3 的檔案；`sidecar/` 沒有任何修改。
- [x] `npm test`（含 `test:sidecar`）、`npm run lint`（含 `runner/`）、兩個 lint exit 0。
- [x] `npm run e2e -- --grep @W6` 通過；`npm run e2e:all`（W0–W6）通過；`npm run e2e:validate` 印 `ok`。
- [x] `summary.md` 貼進 PR；`manifest.json` 的 `redaction.hits` 為空。
- [ ] 在一台有 Claude Code 的主機上手動確認：以 §4.5 的範例啟動 `claude_code` adapter，`adapter.home` 之外沒有新增 Claude Code 的設定檔（§4.6 `CLAUDE_CONFIG_DIR` 的推論）；結果寫進 PR。本次沒有啟動真實 Claude Code。
- [x] 連結檢查通過（`docs/v2` 相對連結 13 個失敗都是 W6 之前就有的：W7 路徑、W0 佔位、W1 的 XSS 範例、W2 的正則）。
- [x] [09](../09-roadmap.md) W6 狀態改為 `VERIFIED`。
