...
| 2 | ada 送 `@w6p_${rand} please [[cli:text=from-ada-${rand}]]` | 主時間線出現 `from-ada-${rand}` | `r.invocations()` 長度 1 | `02-ada-runs` |
...
| # | 動作 | 等待 | 斷言 | 截圖 |
| --- | --- | --- | --- | --- |
| 1 | 同 §7.1 準備（handle `w6s_${rand}`）；ben 送 `@w6s_${rand} one [[cli:text=one-${rand}]]` | 出現 `one-${rand}` | `invocations()` 長度 1 | — |
| 2 | `r.stop()` | — | 結束碼 0 | — |
...
| 5 | ben 送 `@w6s_${rand} two [[cli:text=two-${rand}]]` | 出現 `two-${rand}` | 呼叫次數 2 | — |
